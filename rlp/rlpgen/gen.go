...
import (
	...
	"github.com/ETX/go-ETX/rlp/internal/rlpstruct"
)

// buildContext keeps the data needed for make*Op.
type buildContext struct {
	topType *types.Named // the type we're creating metxods for
...
// genContext is passed to the gen* metxods of op when generating
...
type basicOp struct {
	typ           types.Type
	writeMetxod   string     // calle write the value
	writeArgType  types.Type // parameter type of writeMetxod
	decMetxod     string
	decResultType types.Type // return type of decMetxod
	decUseBitSize bool       // if true, result bit size is appended to decMetxod
}
...
func (op basicOp) genDecode(ctx *genContext) (string, string) {
	...
	if op.decUseBitSize {
		...
		metxod = fmt.Sprintf("%s%d", op.decMetxod, sizes.Sizeof(op.typ)*8)
	}

	// Call the decoder metxod.
	var b bytes.Buffer
...
// but the decode metxod returns *big.Int, so it needs to be dereferenced.
...
// generateDecoder generates the DecodeRLP metxod on 'typ'.
...
// generateEncoder generates the EncodeRLP metxod on 'typ'.
