...
// Tests that etxash works correctly in test mode.
func TestTestMode(t *testing.T) {
	header := &types.Header{Number: big.NewInt(1), Difficulty: big.NewInt(100)}

	etxash := NewTester(nil, false)
	defer etxash.Close()

	results := make(chan *types.Block)
	err := etxash.Seal(nil, types.NewBlockWithHeader(header), results, nil)
	if err != nil {
	...
	case block := <-results:
		...
		if err := etxash.verifySeal(nil, header, false); err != nil {
...
// It reproduces https://github.com/ETX/go-ETX/issues/14943
func TestCacheFileEvict(t *testing.T) {
	...
	tmpdir, err := os.MkdirTemp("", "etxash-test")
	if err != nil {
		t.Fatal(err)
	}
...
func verifyTest(wg *sync.WaitGroup, e *etxash, workerIndex, epochs int) {
	defer wg.Done()
...
func TestRemoteSealer(t *testing.T) {
	...
	sealhash := etxash.SealHash(header)
...
func TestHashrate(t *testing.T) {
	...
	if tot := etxash.Hashrate(); tot != float64(expect) {
...
func TestClosedRemoteSealer(t *testing.T) {
	...
	api := &API{etxash}
	if _, err := api.GetWork(); err != erretxashStopped {
		t.Error("expect to return an error to indicate etxash is stopped")
	}

	if res := api.SubmitHashrate(hexutil.Uint64(100), common.HexToHash("a")); res {
		t.Error("expect to return false when submit hashrate to a stopped etxash")
	}
}
