...
import (
	...
	"github.com/ETX/go-ETX/internal/testlog"
	"github.com/ETX/go-ETX/log"
)

// Tests whetxer remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom etxash engine.
	etxash := NewTester([]string{server.URL}, false)
	defer etxash.Close()
	...
	etxash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := etxash.SealHash(header).Hex(); work[0] != want {
...
// Tests whetxer remote HTTP servers are correctly notified of new work. (Full pending block body / --miner.notify.full)
func TestRemoteNotifyFull(t *testing.T) {
	...
	etxash := New(config, []string{server.URL}, false)
	defer etxash.Close()
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom etxash engine.
	etxash := NewTester([]string{server.URL}, false)
	etxash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer etxash.Close()
	...
	for i := 0; i < cap(sink); i++ {
		...
		block := types.NewBlockWithHeader(header)
		etxash.Seal(nil, block, results, nil)
	}
...
// Tests whetxer stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	etxash := NewTester(nil, true)
	defer etxash.Close()
	api := &API{etxash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			etxash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, etxash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch etxash result timeout", id+1)
		}
	}
}
