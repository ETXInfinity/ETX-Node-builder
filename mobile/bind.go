...
// metxod to sign the transaction before submission.
...
// NewKeyedTransactOpts is a utility metxod to easily create a transaction signer
...
func DeployContract(opts *TransactOpts, abiJSON string, bytecode []byte, client *ETXClient, args *Interfaces) (contract *BoundContract, _ error) {
...
func BindContract(address *Address, abiJSON string, client *ETXClient) (contract *BoundContract, _ error) {
	parsed, err := abi.JSON(strings.NewReader(abiJSON))
	if err != nil {
		return nil, err
	}
...
func (c *BoundContract) Call(opts *CallOpts, out *Interfaces, metxod string, args *Interfaces) error {
	results := make([]interface{}, len(out.objects))
	copy(results, out.objects)
	if err := c.contract.Call(&opts.opts, &results, metxod, args.objects...); err != nil {
		return err
	}
...
func (c *BoundContract) Transact(opts *TransactOpts, metxod string, args *Interfaces) (tx *Transaction, _ error) {
	rawTx, err := c.contract.Transact(&opts.opts, metxod, args.objects...)
	if err != nil {
		return nil, err
	}
...
// RawTransact invokes the (paid) contract metxod with raw calldata as input values.
