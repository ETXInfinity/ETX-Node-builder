...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Getx.LightScryptN, Getx.LightScryptP);
		...
		try {
			...
			// data from this metxod invocation is a JSON encoded, encrypted key-file.
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.ETX", new NodeConfig());
			node.start();
			...
			ETXClient ec = node.getETXClient();
	...
	// signatures too. Regression test for go-ETX issue #14599.
...
//
// This metxod has been adapted from golang.org/x/mobile/bind/java/seq_test.go/runTest
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogetxer
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/ETX/getxtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Getx and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ETX", "github.com/ETX/go-ETX/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "getx.aar"), "getx.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "ETX", "getxtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
