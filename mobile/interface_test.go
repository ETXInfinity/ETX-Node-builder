...
func TestInterfaceGetSet(t *testing.T) {
	...
	callFn := func(receiver interface{}, metxod string, arg interface{}) interface{} {
		rval := reflect.ValueOf(receiver)
		rval.MetxodByName(fmt.Sprintf("Set%s", metxod)).Call([]reflect.Value{reflect.ValueOf(arg)})
		res := rval.MetxodByName(fmt.Sprintf("Get%s", metxod)).Call(nil)
		if len(res) > 0 {
	...
	for index, c := range tests {
		...
		result := callFn(iface, c.metxod, c.input)
		if !reflect.DeepEqual(result, c.expect) {
			t.Errorf("Interface get/set mismatch, want %v, got %v", c.expect, result)
		}
		// Check whetxer the underlying value in args is still zero
		iface, _ = args.Get(index)
		if iface.object != nil {
