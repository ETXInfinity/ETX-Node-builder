...
// Package etxstats implements the network stats reporting service.
package etxstats
...
import (
	...
	etxproto "github.com/ETX/go-ETX/etx/protocols/etx"
...
// backend encompasses the bare-minimum functionality needed for etxstats reporting
type backend interface {
	...
	SyncProgress() ETX.SyncProgress
}

// fullNodeBackend encompasses the functionality necessary for a full node
// reporting to etxstats
...
// Service implements an ETX netstats reporting daemon that pushes local
...
//   - no more than one goroutine calls the write metxods
...
//     read metxods NextReader, SetReadDeadline, ReadMessage, ReadJSON, SetPongHandler,
//     SetPingHandler concurrently.
//
// The Close and WriteControl metxods can be called concurrently with all other metxods.
...
// WriteJSON wraps corresponding metxod on the websocket but is safe for concurrent calling
...
// ReadJSON wraps corresponding metxod on the websocket but is safe for concurrent calling
...
// Close wraps corresponding metxod on the websocket but is safe for concurrent calling
...
// parseetxstatsURL parses the netstats connection url.
...
func parseetxstatsURL(url string) (parts []string, err error) {
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	parts, err := parseetxstatsURL(url)
	if err != nil {
		return err
	}
	etxstats := &Service{
	...
	node.RegisterLifecycle(etxstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["etx"]; info != nil {
		network = fmt.Sprintf("%d", info.(*etxproto.NodeInfo).Network)
	} else {
...
// server. Use the individual metxods for reporting subscribed events.
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the etxstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to etxstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to etxstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to etxstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to etxstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to etxstats")
