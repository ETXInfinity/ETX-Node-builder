...
func TestUnpack(t *testing.T) {
	for i, test := range packUnpackTests {
		t.Run(strconv.Itoa(i)+" "+test.def, func(t *testing.T) {
			//Unpack
			def := fmt.Sprintf(`[{ "name" : "metxod", "type": "function", "outputs": %s}]`, test.def)
			...
			out, err := abi.Unpack("metxod", encb)
			if err != nil {
...
func TestLocalUnpackTests(t *testing.T) {
	for i, test := range unpackTests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			err = abi.UnpackIntoInterface(outptr.Interface(), "metxod", encb)
...
type metxodMultiOutput struct {
	Int    *big.Int
	String string
}

func metxodMultiReturn(require *require.Assertions) (ABI, []byte, metxodMultiOutput) {
	const definition = `[
	{ "name" : "multi", "type": "function", "outputs": [ { "name": "Int", "type": "uint256" }, { "name": "String", "type": "string" } ] }]`
	var expected = metxodMultiOutput{big.NewInt(1), "hello"}
...
func TestMetxodMultiReturn(t *testing.T) {
	...
	abi, data, expected := metxodMultiReturn(require.New(t))
	bigint := new(big.Int)
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.error == "" {
				require.Nil(err, "Should be able to unpack metxod outputs.")
...
func TestMultiReturnWithStringArray(t *testing.T) {
	...
	ret3, ret3Exp := new([2]string), [2]string{"ETX", "Hello, ETX!"}
...
func TestMultiReturnWithStringSlice(t *testing.T) {
	...
	ret1, ret1Exp := new([]string), []string{"ETX", "go-ETX"}
...
func TestUnpackTuple(t *testing.T) {
	...
	type T struct {
		X *big.Int `abi:"x"`
		Z *big.Int `abi:"y"` // Test whetxer the abi tag works.
	}
...
func TestOOMMaliciousInput(t *testing.T) {
	...
	for i, test := range oomTests {
		...
		_, err = abi.Metxods["metxod"].Outputs.UnpackValues(encb)
		if err == nil {
