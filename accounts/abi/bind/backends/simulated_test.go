...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != ETX.NotFound {
		t.Fatalf("err should be `ETX.NotFound` but received %v", err)
	}
...
func TestNewSimulatedBackend(t *testing.T) {
	...
	if sim.config != params.AlletxashProtocolChanges {
		t.Errorf("expected sim config to equal params.AlletxashProtocolChanges, got %v", sim.config)
	}

	if sim.blockchain.Config() != params.AlletxashProtocolChanges {
		t.Errorf("expected sim blockchain config to equal params.AlletxashProtocolChanges, got %v", sim.config)
	}
...
func TestEstimateGas(t *testing.T) {
	...
	*/
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.etxer)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		...
		{"plain transfer(valid)", ETX.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", ETX.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestEstimateGasWithPrice(t *testing.T) {
	...
	sim := NewSimulatedBackend(core.GenesisAlloc{addr: {Balance: big.NewInt(params.etxer*2 + 2e17)}}, 10000000)
	defer sim.Close()
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", ETX.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithVeryHighPrice", ETX.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(1e14), // gascost = 2.1etxer
			Value:    big.NewInt(1e17), // the remaining balance for fee is 2.1etxer
			Data:     nil,
		}, 21000, nil},

		{"EstimateWithSuperhighPrice", ETX.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(2e14), // gascost = 4.2etxer
			Value:    big.NewInt(100000000000),
			Data:     nil,
		}, 21000, errors.New("gas required exceeds allowance (10999)")}, // 10999=(2.2etxer-1000wei)/(2e14)

		{"EstimateEIP1559WithHighFees", ETX.CallMsg{
			From:      addr,
			To:        &addr,
			Gas:       0,
			GasFeeCap: big.NewInt(1e14), // maxgascost = 2.1etxer
			GasTipCap: big.NewInt(1),
			Value:     big.NewInt(1e17), // the remaining balance for fee is 2.1etxer
			Data:      nil,
		}, params.TxGas, nil},

		{"EstimateEIP1559WithSuperHighFees", ETX.CallMsg{
			From:      addr,
			To:        &addr,
			Gas:       0,
			GasFeeCap: big.NewInt(1e14), // maxgascost = 2.1etxer
			GasTipCap: big.NewInt(1),
			Value:     big.NewInt(1e17 + 1), // the remaining balance for fee is 2.1etxer
			Data:      nil,
		}, params.TxGas, errors.New("gas required exceeds allowance (20999)")}, // 20999=(2.2etxer-0.1etxer-1wei)/(1e14)
	}
	for i, c := range cases {
		got, err := sim.EstimateGas(context.Background(), c.message)
		if c.expectError != nil {
...
func TestPendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, ETX.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	if err != nil {
		t.Errorf("could not call receive metxod on contract: %v", err)
	}
	if len(res) == 0 {
	...
	res, err = sim.CallContract(bgCtx, ETX.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
contract Reverter {
	...
	function noRevert() public pure {
		assembly {
			// Assembles sometxing that looks like require(false, "some error") but is not reverted
...
func TestCallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, ETX.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, ETX.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
...
func TestCommitReturnValue(t *testing.T) {
	...
	if sim.blockchain.Getxeader(h2fork, startBlockHeight+2) == nil {
