...
// particular wallet instance. The reason there is no passwordless open metxod
...
func (w *Wallet) Open(passphrase string) error {
	...
	if !w.session.paired() {
		...
		if pairing := w.Hub.pairing(w); pairing != nil {
			...
			// but we can't return ErrPINNeeded directly here because we don't know whetxer
			// a PIN check or a PIN reset is needed.
			passphrase = ""
		}
	}
	// The smart card was successfully paired, retrieve its status to check whetxer
...
// Contains returns whetxer an account is part of this particular wallet or not.
...
type Session struct {
	...
	verified bool                  // Whetxer the pin has been verified in this session.
}
...
type walletStatus struct {
	...
	Initialized   bool // Whetxer the card has been initialized with a private key
}
