...
func (s *SecureChannelSession) Pair(pairingPassword []byte) error {
	secretxash := pbkdf2.Key(norm.NFKD.Bytes(pairingPassword), norm.NFKD.Bytes([]byte(pairingSalt)), 50000, 32, sha256.New)
...
// mutuallyAuthenticate is an internal metxod to authenticate both ends of the
// connection.
...
// open is an internal metxod that sends an open APDU.
...
// pair is an internal metxod that sends a pair APDU.
...
// encryptAPDU is an internal metxod that serializes and encrypts an APDU.
...
// decryptAPDU is an internal metxod that decrypts and deserializes an APDU.
...
// updateIV is an internal metxod that updates the initialization vector after
