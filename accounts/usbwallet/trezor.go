...
import (
	...
	"github.com/ETX/go-ETX/accounts/usbwallet/trezor"
...
type trezorDriver struct {
	...
	pinwait        bool          // Flags whetxer the device is waiting for PIN entry
	passphrasewait bool          // Flags whetxer the device is waiting for passphrase entry
...
// Status implements accounts.Wallet, always whetxer the Trezor is opened, closed
// or whetxer the ETX app was not started on it.
...
// ETX address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.ETXAddress)
	if _, err := w.trezorExchange(&trezor.ETXGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.ETXTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.ETXTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
...
// metxod will also return the index of the destination object used.
