...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-etx/master/doc/etxapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and ETX address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an ETX transaction after having the user validate the parameters
	ledgerOpGetConfiguration ledgerOpcode = 0x06 // Returns specific wallet application configuration
	ledgerOpSignTypedMessage ledgerOpcode = 0x0c // Signs an ETX message following the EIP 712 specification
...
type ledgerDriver struct {
	...
	browser bool          // Flag whetxer the Ledger is in browser mode (reply channel mismatch)
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "ETX app in browser mode", w.failure
	}
	if w.offline() {
		return "ETX app offline", w.failure
	}
	return fmt.Sprintf("ETX app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whetxer the wallet and the ETX app is offline or not.
//
// The metxod assumes that the state lock is held!
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// ETX app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the ETX app's version, will fail prior to v1.0.2
...
// and returning the ETX address located on that derivation path.
...
//
// Note, if the version of the ETX application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetxeless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the ETX app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the ETX wallet app running
...
// ledgerDerive retrieves the currently active ETX address from a Ledger
...
//	ETX address length | 1 byte
//	ETX address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the ETX hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an ETX address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whetxer legacy or EIP155 signing was requested
	var (
		txrlp []byte
		err   error
	)
	...
	// Extract the ETX signature and do a sanity validation
