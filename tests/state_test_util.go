...
// See https://github.com/ETX/EIPs/issues/176 for the test format specification.
...
func (t *StateTest) RunNoVerify(subtest StateSubtest, vmconfig vm.Config, snapshotter bool) (*snapshot.Tree, *state.StateDB, common.Hash, error) {
	...
	if config.IsLondon(new(big.Int)) {
		...
		if baseFee == nil {
			// Retestetx uses `0x10` for genesis baseFee. Therefore, it defaults to
	...
	context.Getxash = vmTestBlockHash
...
func MakePreState(db etxdb.Database, accounts core.GenesisAlloc, snapshotter bool) (*snapshot.Tree, *state.StateDB) {
...
func (tx *stTransaction) toMessage(ps stPostState, baseFee *big.Int) (core.Message, error) {
	...
	// Value, Data hex encoding is messy: https://github.com/ETX/tests/issues/203
	value := new(big.Int)
	if valueHex != "0x" {
