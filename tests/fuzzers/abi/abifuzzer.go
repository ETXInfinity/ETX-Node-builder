...
func unpackPack(abi abi.ABI, metxod string, input []byte) ([]interface{}, bool) {
	if out, err := abi.Unpack(metxod, input); err == nil {
		_, err := abi.Pack(metxod, out...)
		if err != nil {
...
func packUnpack(abi abi.ABI, metxod string, input *[]interface{}) bool {
	if packed, err := abi.Pack(metxod, input); err == nil {
		outptr := reflect.New(reflect.TypeOf(input))
		err := abi.UnpackIntoInterface(outptr.Interface(), metxod, packed)
		if err != nil {
			panic(err)
		}
