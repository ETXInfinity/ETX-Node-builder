...
var (
	...
	bankFunds  = new(big.Int).Mul(big.NewInt(100), big.NewInt(params.etxer))
...
func makechain() (bc *core.BlockChain, addrHashes, txHashes []common.Hash) {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(gspec, etxash.NewFaker(), testChainLen,
	...
	bc, _ = core.NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, etxash.NewFaker(), vm.Config{}, nil, nil)
...
func (f *fuzzer) GetxelperTrie(typ uint, index uint64) *trie.Trie {
	if typ == 0 {
		return f.chtTrie
	} else if typ == 1 {
		return f.bloomTrie
...
func Fuzz(input []byte) int {
	...
	for !f.exhausted {
		...
		case 5:
			req := &l.GetxelperTrieProofsPacket{Reqs: make([]l.HelperTrieReq, f.randomInt(l.MaxHelperTrieProofsFetch+1))}
			for i := range req.Reqs {
			...
			f.doFuzz(l.GetxelperTrieProofsMsg, req)
