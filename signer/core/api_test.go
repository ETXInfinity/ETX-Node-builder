...
func (ui *headlessUi) OnApprovedTx(tx etxapi.SignTransactionResult) {}
...
func TestSignTx(t *testing.T) {
	var (
		list      []common.Address
		res, res2 *etxapi.SignTransactionResult
		err       error
	)
	...
	res, err = api.SignTransaction(context.Background(), tx, &metxodSig)
	if res != nil {
	...
	res2, err = api.SignTransaction(context.Background(), tx, &metxodSig)
	if err != nil {
		t.Fatal(err)
	}
