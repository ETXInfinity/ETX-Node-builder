...
// determineSignatureFormat determines which signature metxod should be used based upon the mime type
...
// This metxod returns the mimetype for signing along with the request
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	var (
		req          *SignDataRequest
		useETXV = true // Default to use V = 27 or 28, the legacy ETX format
	)
	mediaType, _, err := mime.ParseMediaType(contentType)
	if err != nil {
		return nil, useETXV, err
	}
	...
	case apitypes.ApplicationClique.Mime:
		// Clique is the ETX PoA standard
		stringData, ok := data.(string)
		if !ok {
			return nil, useETXV, fmt.Errorf("input for %v must be an hex-encoded string", apitypes.ApplicationClique.Mime)
		}
	...
	default: // also case TextPlain.Mime:
		// Calculates an ETX ECDSA signature for:
		// hash = keccak256("\x19ETX Signed Message:\n${message length}${message}")
		// We expect it to be a string
		if stringData, ok := data.(string); !ok {
			return nil, useETXV, fmt.Errorf("input for text/plain must be an hex-encoded string")
		} else {
...
//
// The metxod requires the extra data to be at least 65 bytes -- the original implementation
