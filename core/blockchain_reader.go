...
// caching it if found.
func (bc *BlockChain) Getxeader(hash common.Hash, number uint64) *types.Header {
	return bc.hc.Getxeader(hash, number)
}

// GetxeaderByHash retrieves a block header from the database by hash, caching it if
// found.
func (bc *BlockChain) GetxeaderByHash(hash common.Hash) *types.Header {
	return bc.hc.GetxeaderByHash(hash)
}

// GetxeaderByNumber retrieves a block header from the database by number,
// caching it (associated with its hash) if found.
func (bc *BlockChain) GetxeaderByNumber(number uint64) *types.Header {
	return bc.hc.GetxeaderByNumber(number)
}

// GetxeadersFrom returns a contiguous segment of headers, in rlp-form, going
// backwards from the given number.
func (bc *BlockChain) GetxeadersFrom(number, count uint64) []rlp.RawValue {
	return bc.hc.GetxeadersFrom(number, count)
}
