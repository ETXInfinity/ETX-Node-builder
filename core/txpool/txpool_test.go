...
func (c *testChain) State() (*state.StateDB, error) {
	...
	if *c.trigger {
		...
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.etxer))
		*c.trigger = false
	}
	return stdb, nil
}
...
// state reset and tests whetxer the pending state is in sync with the
...
func TestStateChangeDuringReset(t *testing.T) {
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.etxer))
...
// Tests whetxer highest fee cap transaction is retained after a batch of high effective
