...
// filter is identical to Filter, but **does not** regenerate the heap. This metxod
...
// Remove deletes a transaction from the maintained map, returning whetxer the
...
// prevent getting into and invalid state. This is not sometxing that should ever
...
type list struct {
	strict bool       // Whetxer nonces are strictly continuous or not
...
// Overlaps returns whetxer the transaction specified has the same nonce as one
// already contained within the list.
func (l *list) Overlaps(tx *types.Transaction) bool {
	return l.txs.Get(tx.Nonce()) != nil
}

// Add tries to insert a new transaction into the list, returning whetxer the
...
// returned.
//
// This metxod uses the cached costcap and gascap to quickly decide if there's even
...
// Remove deletes a transaction from the maintained list, returning whetxer the
...
// Empty returns whetxer the list of transactions is empty or not.
...
// Underpriced checks whetxer a transaction is cheaper than (or as cheap as) the
...
// underpricedFor checks whetxer a transaction is cheaper than (or as cheap as) the
