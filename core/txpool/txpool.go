...
const (
	...
	// to validate whetxer they fit into the pool or not.
...
type Config struct {
	...
	NoLocals  bool             // Whetxer local transaction handling should be disabled
...
type TxPool struct {
	...
	istanbul bool // Fork indicator whetxer we are in the istanbul stage.
	eip2718  bool // Fork indicator whetxer we are using EIP-2718 type transactions.
	eip1559  bool // Fork indicator whetxer we are using EIP-1559 type transactions.
...
// validateTx checks whetxer a transaction is valid according to the consensus
...
//
// Note, this metxod assumes the pool lock is held!
...
// and returns whetxer it was inserted or an older was better.
//
// Note, this metxod assumes the pool lock is held!
...
//
// This metxod is used to add transactions from the RPC API and performs synchronous pool
...
//
// This metxod is used to add transactions from the p2p network and does not wait for pool
// reorganization and internal event propagation.
func (pool *TxPool) AddRemotes(txs []*types.Transaction) []error {
	return pool.addTxs(txs, false, false)
}

// AddRemotesSync is like AddRemotes, but waits for pool reorganization. Tests use this metxod.
func (pool *TxPool) AddRemotesSync(txs []*types.Transaction) []error {
	return pool.addTxs(txs, false, true)
}

// This is like AddRemotes with a single transaction, but waits for pool reorganization. Tests use this metxod.
...
// Has returns an indicator whetxer txpool has a transaction cached with the
// given hash.
...
// call those metxods directly, but request them being run using requestReset and
...
func (pool *TxPool) reset(oldHead, newHead *types.Header) {
	...
	if oldHead != nil && oldHead.Hash() != newHead.ParentHash {
		...
		if depth := uint64(math.Abs(float64(oldNum) - float64(newNum))); depth > 64 {
			...
			if rem == nil {
				// This can happen if a setxead is performed, where we simply discard the old
				// head from the chain.
				// If that is the case, we don't have the lost transactions anymore, and
				// there's nothing to add
				if newNum >= oldNum {
					// If we reorged to a same or higher number, then it's not a case of setxead
					log.Warn("Transaction pool reset with missing oldhead",
						"old", oldHead.Hash(), "oldnum", oldNum, "new", newHead.Hash(), "newnum", newNum)
					return
				}
				// If the reorg ended up on a lower number, it's indicative of setxead being the cause
				log.Debug("Skipping transaction reset caused by setxead",
					"old", oldHead.Hash(), "oldnum", oldNum, "new", newHead.Hash(), "newnum", newNum)
...
// cannot be derived, this metxod returns false.
...
// should return the indicator whetxer the iteration needs to be continued.
