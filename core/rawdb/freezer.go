...
//     reserving it for go-ETX. This would also reduce the memory requirements
//     of Getx, and thus also GC overhead.
type Freezer struct {
...
// HasAncient returns an indicator whetxer the specified ancient data exists
// in the freezer.
...
func (f *Freezer) ReadAncients(fn func(etxdb.AncientReaderOp) error) (err error) {
	f.writeLock.RLock()
	defer f.writeLock.RUnlock()
...
func (f *Freezer) ModifyAncients(fn func(etxdb.AncientWriteOp) error) (writeSize int64, err error) {
	if f.readonly {
