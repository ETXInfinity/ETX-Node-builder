...
func ReadSnapshotDisabled(db etxdb.KeyValueReader) bool {
	disabled, _ := db.Has(snapshotDisabledKey)
	return disabled
}

// WriteSnapshotDisabled stores the snapshot pause flag.
func WriteSnapshotDisabled(db etxdb.KeyValueWriter) {
...
func DeleteSnapshotDisabled(db etxdb.KeyValueWriter) {
...
func ReadSnapshotRoot(db etxdb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db etxdb.KeyValueWriter, root common.Hash) {
...
// the persisted snapshot. Since snapshots are not immutable, this  metxod can
// be used during updates, so a crash or failure will mark the entire snapshot
// invalid.
func DeleteSnapshotRoot(db etxdb.KeyValueWriter) {
...
func ReadAccountSnapshot(db etxdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db etxdb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db etxdb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db etxdb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db etxdb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db etxdb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db etxdb.Iteratee, accountHash common.Hash) etxdb.Iterator {
...
func ReadSnapshotJournal(db etxdb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db etxdb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db etxdb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db etxdb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db etxdb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db etxdb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db etxdb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db etxdb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db etxdb.KeyValueWriter) {
...
func ReadSnapshotSyncStatus(db etxdb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db etxdb.KeyValueWriter, status []byte) {
