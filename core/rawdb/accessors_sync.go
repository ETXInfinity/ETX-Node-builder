...
func ReadSkeletonSyncStatus(db etxdb.KeyValueReader) []byte {
	data, _ := db.Get(skeletonSyncStatusKey)
	return data
}

// WriteSkeletonSyncStatus stores the serialized sync status to save at shutdown.
func WriteSkeletonSyncStatus(db etxdb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSkeletonSyncStatus(db etxdb.KeyValueWriter) {
...
func ReadSkeletonHeader(db etxdb.KeyValueReader, number uint64) *types.Header {
...
func WriteSkeletonHeader(db etxdb.KeyValueWriter, header *types.Header) {
...
func DeleteSkeletonHeader(db etxdb.KeyValueWriter, number uint64) {
