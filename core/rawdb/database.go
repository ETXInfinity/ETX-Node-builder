...
import (
	...
	"github.com/ETX/go-ETX/etxdb/leveldb"
	"github.com/ETX/go-ETX/etxdb/memorydb"
...
// Freeze is a helper metxod used for external testing to trigger and block until
...
func (db *nofreezedb) ModifyAncients(func(etxdb.AncientWriteOp) error) (int64, error) {
	return 0, errNotSupported
}
...
func (db *nofreezedb) ReadAncients(fn func(reader etxdb.AncientReaderOp) error) (err error) {
	// Unlike other ancient-related metxods, this metxod does not return
	// errNotSupported when invoked.
	// The reason for this is that the caller might want to do several things:
	// 1. Check if sometxing is in freezer,
...
func NewDatabase(db etxdb.KeyValueStore) etxdb.Database {
...
func NewDatabaseWithFreezer(db etxdb.KeyValueStore, ancient string, namespace string, readonly bool) (etxdb.Database, error) {
	...
	// validate in this metxod. If, however, the genesis hash is not nil, compare
	// it to the freezer content.
	if kvgenesis, _ := db.Get(headerHashKey(0)); len(kvgenesis) > 0 {
		if frozen, _ := frdb.Ancients(); frozen > 0 {
			// If the freezer already contains sometxing, ensure that the genesis blocks
...
func NewMemoryDatabase() etxdb.Database {
...
func NewMemoryDatabaseWithCap(size int) etxdb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string, readonly bool) (etxdb.Database, error) {
...
// opened.
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, ancient string, namespace string, readonly bool) (etxdb.Database, error) {
...
func InspectDatabase(db etxdb.Database, keyPrefix, keyStart []byte) error {
	...
	table.Setxeader([]string{"Database", "Category", "Size", "Items"})
