...
func ReadDatabaseVersion(db etxdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db etxdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db etxdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db etxdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
// given genesis hash.
func ReadGenesisStateSpec(db etxdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(genesisStateSpecKey(hash))
	return data
}

// WriteGenesisStateSpec writes the genesis state specification into the disk.
func WriteGenesisStateSpec(db etxdb.KeyValueWriter, hash common.Hash, data []byte) {
...
func PushUncleanShutdownMarker(db etxdb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db etxdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func UpdateUncleanShutdownMarker(db etxdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
// ReadTransitionStatus retrieves the etx2 transition status from the database
func ReadTransitionStatus(db etxdb.KeyValueReader) []byte {
	data, _ := db.Get(transitionStatusKey)
	return data
}

// WriteTransitionStatus stores the etx2 transition status to the database
func WriteTransitionStatus(db etxdb.KeyValueWriter, data []byte) {
	if err := db.Put(transitionStatusKey, data); err != nil {
		log.Crit("Failed to store the etx2 transition status", "err", err)
	}
}
