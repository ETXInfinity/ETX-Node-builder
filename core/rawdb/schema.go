...
var (
	...
	// lastPivotKey tracks the last pivot block used by fast sync (to reenable on setxead).
	...
	// transitionStatusKey tracks the etx2 transition status.
	transitionStatusKey = []byte("etx2-transition")
	...
	configPrefix   = []byte("ETX-config-")  // config prefix for the db
	genesisPrefix  = []byte("ETX-genesis-") // genesis state prefix for the db
...
// IsCodeKey reports whetxer the given byte slice is the key of contract code,
