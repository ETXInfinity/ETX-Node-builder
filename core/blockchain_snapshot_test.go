...
// It can be used for testing that restart Getx normally.
...
// It can be used for testing that restart Getx after the crash.
...
// setxeadSnapshotTest is the test type used to test this scenario:
// - have a complete snapshot
// - set the head to a lower point
// - restart
type setxeadSnapshotTest struct {
	snapshotTestBasic
	setxead uint64 // Block number to set head back to
}

func (snaptest *setxeadSnapshotTest) test(t *testing.T) {
	// It's hard to follow the test case, visualize the input
	// log.Root().Setxandler(log.LvlFilterHandler(log.LvlTrace, log.StreamHandler(os.Stderr, log.TerminalFormat(true))))
	// fmt.Println(tt.dump())
	chain, blocks := snaptest.prepare(t)

	// Rewind the chain if setxead operation is required.
	chain.Setxead(snaptest.setxead)
	chain.Stop()
...
// Tests a Getx restart with valid snapshot. Before the shutdown, all snapshot
...
// Tests a Getx was crashed and restarts with a broken snapshot. In this case the
...
// Tests a Getx was crashed and restarts with a broken snapshot. In this case
...
// Tests a Getx was running with snapshot enabled. Then restarts without
...
// Tests the Getx was running with snapshot enabled and resetxead is applied.
...
func TestSetxeadWithNewSnapshot(t *testing.T) {
	// Chain:
	//   G->C1->C2->C3->C4->C5->C6->C7->C8 (HEAD)
	//
	// Commit:   G
...
// Tests the Getx was running with a complete snapshot and then imports a few
