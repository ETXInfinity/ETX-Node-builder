...
func generateSnapshot(diskdb etxdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash) *diskLayer {
...
func journalProgress(db etxdb.KeyValueWriter, marker []byte, stats *generatorStats) {
...
type proofResult struct {
	...
	proofErr error      // Indicator whetxer the given state range is valid or not
...
// last returns the last verified element key regardless of whetxer the range proof is
...
func (dl *diskLayer) generateRange(ctx *generatorContext, trieId *trie.ID, prefix []byte, kind string, origin []byte, max int, onState onStateCallback, valueConvertFn func([]byte) ([]byte, error)) (bool, []byte, error) {
	...
	var snapNodeCache etxdb.KeyValueStore
	if len(result.keys) > 0 {
...
func (dl *diskLayer) checkAndFlush(ctx *generatorContext, current []byte) error {
	...
	if ctx.batch.ValueSize() > etxdb.IdealBatchSize || abort != nil {
...
// gathering and logging, since the metxod surfs the blocks as they arrive, often
// being restarted.
