...
func CheckDanglingStorage(chaindb etxdb.KeyValueStore) error {
...
func checkDanglingDiskStorage(chaindb etxdb.KeyValueStore) error {
	var (
		lastReport = time.Now()
		start      = time.Now()
		lastKey    []byte
...
// snapshot difflayers.
func checkDanglingMemStorage(db etxdb.KeyValueStore) error {
	start := time.Now()
...
func CheckJournalAccount(db etxdb.KeyValueStore, hash common.Hash) error {
