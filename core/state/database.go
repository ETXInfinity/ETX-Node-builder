...
type Database interface {
	...
	DiskDB() etxdb.KeyValueStore

	// TrieDB retrieves the low level trie database used for data storage.
	TrieDB() *trie.Database
}

// Trie is a ETX Merkle Patricia trie.
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb etxdb.KeyValueWriter) error
}

// NewDatabase creates a backing store for state. The returned database is safe for
// concurrent use, but does not retain any recent trie nodes in memory. To keep some
// historical state in memory, use the NewDatabaseWithConfig constructor.
func NewDatabase(db etxdb.Database) Database {
...
// large memory cache.
func NewDatabaseWithConfig(db etxdb.Database, config *trie.Config) Database {
	return &cachingDB{
...
// DiskDB returns the underlying key-value disk database.
func (db *cachingDB) DiskDB() etxdb.KeyValueStore {
	return db.disk
}
