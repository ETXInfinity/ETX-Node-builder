...
// Package state provides a caching layer atop the ETX state trie.
package state
...
// StateDB structs within the ETX protocol are used to store anything
...
// This metxod will panic if the refund counter goes below zero
...
// Exist reports whetxer the given account address exists in the state.
// Notably this also returns true for suicided accounts.
func (s *StateDB) Exist(addr common.Address) bool {
	return s.getStateObject(addr) != nil
}

// Empty returns whetxer the state object is either non-existent
...
//
// Setting, updating & deleting state object metxods.
//
...
//
// Carrying over the balance ensures that etxer doesn't disappear.
...
func (s *StateDB) Copy() *StateDB {
	...
	for addr := range s.journal.dirties {
		// As documented [here](https://github.com/ETX/go-ETX/pull/16485#issuecomment-380438527),
		// and in the Finalise-metxod, there is a case where an object is in the journal but not
...
// This metxod must be invoked before state transition.
//
// Berlin fork:
