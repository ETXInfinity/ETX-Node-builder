...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	filterSystem := filters.NewFilterSystem(etxservice.APIBackend, filters.Config{})
...
func generateTestChain() (*core.Genesis, []*types.Block) {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(genesis, etxash.NewFaker(), 1, generate)
...
func TestGetxClient(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			"TestSetxead",
			func(t *testing.T) { testSetxead(t, client) },
		}, {
...
func testGetProof(t *testing.T, client *rpc.Client) {
	ec := New(client)
	etxcl := etxclient.NewClient(client)
	...
	// test nonce
	nonce, _ := etxcl.NonceAt(context.Background(), result.Address, nil)
	if result.Nonce != nonce {
		t.Fatalf("invalid nonce, want: %v got: %v", nonce, result.Nonce)
	}
	// test balance
	balance, _ := etxcl.BalanceAt(context.Background(), result.Address, nil)
	...
	slotValue, _ := etxcl.StorageAt(context.Background(), testAddr, testSlot, nil)
...
func testSetxead(t *testing.T, client *rpc.Client) {
	ec := New(client)
	err := ec.Setxead(context.Background(), big.NewInt(0))
	if err != nil {
		t.Fatal(err)
	}
}
...
func testSubscribePendingTransactions(t *testing.T, client *rpc.Client) {
	...
	// Send a transaction
	chainID, err := etxcl.ChainID(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	// Create transaction
	...
	// Send transaction
	err = etxcl.SendTransaction(context.Background(), signedTx)
	if err != nil {
		t.Fatal(err)
	}
