...
// Package getxclient provides an RPC client for getx-specific APIs.
package getxclient
...
// Client is a wrapper around rpc.Client that implements getx-specific functionality.
//
// If you want to use the standardized ETX RPC functionality, use etxclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg ETX.CallMsg) (*types.AccessList, uint64, string, error) {
	...
	if err := ec.c.CallContext(ctx, &result, "etx_createAccessList", toCallArg(msg)); err != nil {
		return nil, 0, "", err
	}
...
func (ec *Client) GetProof(ctx context.Context, account common.Address, keys []string, blockNumber *big.Int) (*AccountResult, error) {
	...
	err := ec.c.CallContext(ctx, &res, "etx_getProof", account, keys, toBlockNumArg(blockNumber))
...
// the message call.
// Please use etxclient.CallContract instead if you don't need the override functionality.
func (ec *Client) CallContract(ctx context.Context, msg ETX.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
	var hex hexutil.Bytes
	err := ec.c.CallContext(
		ctx, &hex, "etx_call", toCallArg(msg),
		toBlockNumArg(blockNumber), overrides,
	)
	return hex, err
}

// GCStats retrieves the current garbage collection stats from a getx node.
...
// MemStats retrieves the current memory stats from a getx node.
...
// Setxead sets the current head of the local chain by block number.
// Note, this is a destructive action and may severely damage your chain.
// Use with extreme caution.
func (ec *Client) Setxead(ctx context.Context, number *big.Int) error {
	return ec.c.CallContext(ctx, nil, "debug_setxead", toBlockNumArg(number))
}

// GetNodeInfo retrieves the node info of a getx node.
...
func (ec *Client) SubscribeFullPendingTransactions(ctx context.Context, ch chan<- *types.Transaction) (*rpc.ClientSubscription, error) {
	return ec.c.etxSubscribe(ctx, ch, "newPendingTransactions", true)
}

// SubscribePendingTransactions subscribes to new pending transaction hashes.
func (ec *Client) SubscribePendingTransactions(ctx context.Context, ch chan<- common.Hash) (*rpc.ClientSubscription, error) {
	return ec.c.etxSubscribe(ctx, ch, "newPendingTransactions")
}
