...
// Verify that Client implements the ETX interfaces.
var (
	_ = ETX.ChainReader(&Client{})
	_ = ETX.TransactionReader(&Client{})
	_ = ETX.ChainStateReader(&Client{})
	_ = ETX.ChainSyncReader(&Client{})
	_ = ETX.ContractCaller(&Client{})
	_ = ETX.GasEstimator(&Client{})
	_ = ETX.GasPricer(&Client{})
	_ = ETX.LogFilterer(&Client{})
	_ = ETX.PendingStateReader(&Client{})
	// _ = ETX.PendingStateEventer(&Client{})
	_ = ETX.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create ETX Service
	config := &etxconfig.Config{Genesis: genesis}
	config.etxash.PowMode = etxash.ModeFake
	etxservice, err := etx.New(n, config)
	if err != nil {
		t.Fatalf("can't create new ETX service: %v", err)
	}
	// Import the test chain.
	if err := n.Start(); err != nil {
		t.Fatalf("can't start test node: %v", err)
	}
	if _, err := etxservice.BlockChain().InsertChain(blocks[1:]); err != nil {
...
func generateTestChain() []*types.Block {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(genesis, etxash.NewFaker(), 2, generate)
	return append([]*types.Block{genesis.ToBlock()}, blocks...)
}

func TestetxClient(t *testing.T) {
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == ETX.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != ETX.NotFound {
		t.Fatal("error should be ETX.NotFound")
	}
}
...
func testChainID(t *testing.T, client *rpc.Client) {
	...
	if id == nil || id.Cmp(params.AlletxashProtocolChanges.ChainID) != 0 {
