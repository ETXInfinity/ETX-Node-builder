...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of getx is "getx". If no
	...
	// registered services, instead those can use utility metxods to create/access
...
// ExtRPCEnabled returns the indicator whetxer node enables the external
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Getx", keep that.
	if name == "getx" || name == "getx-testnet" {
		name = "Getx"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "getx" instances.
var isOldGetxResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by getx 1.4 are used if they exist.
	if warn, isOld := isOldGetxResource[path]; isOld {
		oldpath := ""
		if c.name() == "getx" {
			oldpath = filepath.Join(c.DataDir, path)
		}
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGetxResourceWarning {
				c.oldGetxResourceWarning = true
				log.Warn("Using deprecated resource file, please move this file to the 'getx' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
...
func (c *Config) checkLegacyFile(path string) {
	...
	default:
		// We shouldn't wind up here, but better print sometxing just in case.
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-ETX-keystore")
		isEphemeral = true
	}
