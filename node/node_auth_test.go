...
func (at *authTest) Run(t *testing.T) {
	...
	if x != "hello engine" {
		t.Fatalf("metxod was silent but did not return expected value: %q", x)
	}

	err = cl.CallContext(ctx, &x, "etx_helloWorld")
...
func TestAuthEndpoints(t *testing.T) {
	...
	// Getx must read it from a file, and does not support in-memory JWT secrets, so we create a temporary file.
	...
	conf := &Config{
		...
		WSModules:   []string{"etx", "engine"},
		HTTPModules: []string{"etx", "engine"},
	}
	node, err := New(conf)
	if err != nil {
...
func noneAuth(secret [32]byte) rpc.HTTPAuth {
	return func(header http.Header) error {
		token := jwt.NewWithClaims(jwt.SigningMetxodNone, jwt.MapClaims{
...
func offsetTimeAuth(secret [32]byte, offset time.Duration) rpc.HTTPAuth {
	return func(header http.Header) error {
		token := jwt.NewWithClaims(jwt.SigningMetxodHS256, jwt.MapClaims{
