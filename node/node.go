...
import (
	...
	"github.com/prometxeus/tsdb/fileutil"
)
...
func New(conf *Config) (*Node, error) {
	...
	// Creates an empty AccountManager with no backends. Callers (e.g. cmd/getx)
...
// startRPC is a helper metxod to configure all the various RPC endpoints during node
...
// Server retrieves the currently running P2P network layer. This metxod is meant
...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string, readonly bool) (etxdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, ancient string, namespace string, readonly bool) (etxdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
// closeTrackingDB wraps the Close metxod of a database. When the database is closed by the
...
func (n *Node) wrapDatabase(db etxdb.Database) etxdb.Database {
