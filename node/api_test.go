...
// checking whetxer the HTTP server is started correctly.
func TestStartRPC(t *testing.T) {
	type test struct {
		...
		wantReachable bool // whetxer the HTTP server should be reachable at all
		wantHandlers  bool // whetxer RegisterHandler handlers should be accessible
		wantRPC       bool // whetxer JSON-RPC/HTTP should be accessible
		wantWS        bool // whetxer JSON-RPC/WS should be accessible
	}
...
// checkBodyOK checks whetxer the given HTTP URL responds with 200 OK and body "OK".
...
// checkRPC checks whetxer JSON-RPC works against the given URL.
