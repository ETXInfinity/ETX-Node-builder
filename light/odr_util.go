...
// GetxeaderByNumber retrieves the canonical block header corresponding to the
// given number. The returned header is proven by local CHT.
func GetxeaderByNumber(ctx context.Context, odr OdrBackend, number uint64) (*types.Header, error) {
...
func GetCanonicalHash(ctx context.Context, odr OdrBackend, number uint64) (common.Hash, error) {
	...
	header, err := GetxeaderByNumber(ctx, odr, number)
	if err != nil {
...
func GetTransaction(ctx context.Context, odr OdrBackend, txHash common.Hash) (*types.Transaction, common.Hash, uint64, uint64, error) {
	...
	if header, err := GetxeaderByNumber(ctx, odr, pos.BlockIndex); err != nil || header.Hash() != pos.BlockHash {
		return nil, common.Hash{}, 0, 0, err
	}
