...
type TxPool struct {
	...
	eip2718  bool // Fork indicator whetxer we are in the eip2718 stage.
}

// TxRelayBackend provides an interface to the mechanism that forwards transactions to the
// etx network. The implementations of the functions should be non-blocking.
//
...
func (pool *TxPool) reorgOnNewHead(ctx context.Context, newHeader *types.Header) (txStateChanges, error) {
	...
	oldh := pool.chain.GetxeaderByHash(pool.head)
	newh := newHeader
	...
	for oldh.Hash() != newh.Hash() {
		if oldh.Number.Uint64() >= newh.Number.Uint64() {
			...
			oldh = pool.chain.Getxeader(oldh.ParentHash, oldh.Number.Uint64()-1)
		}
		if oldh.Number.Uint64() < newh.Number.Uint64() {
			newHashes = append(newHashes, newh.Hash())
			newh = pool.chain.Getxeader(newh.ParentHash, newh.Number.Uint64()-1)
			if newh == nil {
...
// validateTx checks whetxer a transaction is valid according to the consensus rules.
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	// block limit gas.
	header := pool.chain.GetxeaderByHash(pool.head)
