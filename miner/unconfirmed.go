...
// chainRetriever is used by the unconfirmed block set to verify whetxer a previously
// mined block is part of the canonical chain or not.
type chainRetriever interface {
	// GetxeaderByNumber retrieves the canonical header associated with a block number.
...
func (set *unconfirmedBlocks) Shift(height uint64) {
	...
	for set.blocks != nil {
		...
		header := set.chain.GetxeaderByNumber(next.index)
		switch {
		case header == nil:
		...
		default:
			// Block is not canonical, check whetxer we have an uncle or a lost block
			included := false
