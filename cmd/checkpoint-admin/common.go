...
func newClient(ctx *cli.Context) *etxclient.Client {
	client, err := etxclient.Dial(ctx.String(nodeURLFlag.Name))
	if err != nil {
		utils.Fatalf("Failed to connect to ETX node: %v", err)
	}
	return client
}
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, etxclient.NewClient(client))
	if err != nil {
