...
import (
	...
	"github.com/ETX/go-ETX/contracts/checkpointoracle"
...
func sign(ctx *cli.Context) error {
	var (
		offline bool // The indicator whetxer we sign checkpoint by offline.
		chash   common.Hash
		cindex  uint64
	...
	if !ctx.IsSet(nodeURLFlag.Name) {
		...
		head, err := etxclient.NewClient(node).HeaderByNumber(reqCtx, nil)
		if err != nil {
			return err
		}
	...
	// isAdmin checks whetxer the specified signer is admin.
...
func publish(ctx *cli.Context) error {
	...
	head, err := etxclient.NewClient(client).HeaderByNumber(reqCtx, nil)
	if err != nil {
		return err
	}
	num := head.Number.Uint64()
	recent, err := etxclient.NewClient(client).HeaderByNumber(reqCtx, big.NewInt(int64(num-128)))
	if err != nil {
		return err
	}
