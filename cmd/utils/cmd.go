...
func StartNode(ctx *cli.Context, stack *node.Node, isConsole bool) {
	...
	go func() {
		...
		minFreeDiskSpace := 2 * etxconfig.Defaults.TrieDirtyCache // Default 2 * 256Mb
...
func monitorFreeDiskSpace(sigc chan os.Signal, path string, freeDiskSpaceCritical uint64) {
	for {
		...
		if freeSpace < freeDiskSpaceCritical {
			log.Error("Low disk space. Gracefully shutting down Getx to prevent database corruption.", "available", common.StorageSize(freeSpace))
			sigc <- syscall.SIGTERM
			break
		} else if freeSpace < 2*freeDiskSpaceCritical {
			log.Warn("Disk space is running low. Getx will shutdown if disk space runs below critical level.", "available", common.StorageSize(freeSpace), "critical_level", common.StorageSize(freeDiskSpaceCritical))
		}
...
func ImportPreimages(db etxdb.Database, fn string) error {
...
// It's a part of the deprecated functionality, should be removed in the future.
func ExportPreimages(db etxdb.Database, fn string) error {
...
type exportHeader struct {
	Magic    string // Always set to 'getxdbdump' for disambiguation
	Version  uint64
	Kind     string
	UnixTime uint64
}

const exportMagic = "getxdbdump"
const (
	OpBatchAdd = 0
	OpBatchDel = 1
)
...
func ImportLDBData(db etxdb.Database, f string, startIndex int64, interrupt chan struct{}) error {
