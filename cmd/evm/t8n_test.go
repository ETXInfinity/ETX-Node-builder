...
import (
	...
	"github.com/ETX/go-ETX/cmd/evm/internal/t8ntool"
	"github.com/ETX/go-ETX/internal/cmdtest"
)

func TestMain(m *testing.M) {
	// Run the app if we've been exec'd as "etxkey-test" in runetxkey.
...
func (args *b11rInput) get(base string) []string {
	...
	if args.etxash {
		out = append(out, "--seal.etxash")
	}
	if opt := args.etxashMode; opt != "" {
		out = append(out, "--seal.etxash.mode")
		out = append(out, fmt.Sprintf("%v/%v", base, opt))
	}
	if opt := args.etxashDir; opt != "" {
		out = append(out, "--seal.etxash.dir")
		out = append(out, fmt.Sprintf("%v/%v", base, opt))
	}
