...
state generators can swap between a `getx`-based implementation and a `parityvm`-based
implementation.
...
- `state.reward`
  - For etxash, it is `5000000000000000000` `wei`,
...
ERROR(4): getxash(3) invoked, blockhash for that block not provided
```
Error code: 4
