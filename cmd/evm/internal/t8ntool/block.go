...
type bbInput struct {
	...
	etxash    bool                 `json:"-"`
	etxashDir string               `json:"-"`
	PowMode   etxash.Mode          `json:"-"`
...
// sealetxash seals the given block using etxash.
func (i *bbInput) sealetxash(block *types.Block) (*types.Block, error) {
	if i.Header.Nonce != nil {
		return nil, NewError(ErrorConfig, fmt.Errorf("sealing with etxash will overwrite provided nonce"))
	}
	etxashConfig := etxash.Config{
		PowMode:        i.PowMode,
		DatasetDir:     i.etxashDir,
		CacheDir:       i.etxashDir,
		DatasetsInMem:  1,
		DatasetsOnDisk: 2,
		CachesInMem:    2,
		CachesOnDisk:   3,
	}
	engine := etxash.New(etxashConfig, nil, true)
	defer engine.Close()
...
func BuildBlock(ctx *cli.Context) error {
	// Configure the go-ETX logger
	glogger := log.NewGlogHandler(log.StreamHandler(os.Stderr, log.TerminalFormat(false)))
	glogger.Verbosity(log.Lvl(ctx.Int(VerbosityFlag.Name)))
	log.Root().Setxandler(glogger)
...
func readInput(ctx *cli.Context) (*bbInput, error) {
	var (
		...
		etxashOn   = ctx.Bool(SealetxashFlag.Name)
		etxashDir  = ctx.String(SealetxashDirFlag.Name)
		etxashMode = ctx.String(SealetxashModeFlag.Name)
		inputData  = &bbInput{}
	)
	if etxashOn && cliqueStr != "" {
		return nil, NewError(ErrorConfig, fmt.Errorf("both etxash and clique sealing specified, only one may be chosen"))
	}
	if etxashOn {
		inputData.etxash = etxashOn
		inputData.etxashDir = etxashDir
		switch etxashMode {
		case "normal":
			inputData.PowMode = etxash.ModeNormal
		case "test":
			inputData.PowMode = etxash.ModeTest
		case "fake":
			inputData.PowMode = etxash.ModeFake
		default:
			return nil, NewError(ErrorConfig, fmt.Errorf("unknown pow mode: %s, supported modes: test, fake, normal", etxashMode))
		}
	}
