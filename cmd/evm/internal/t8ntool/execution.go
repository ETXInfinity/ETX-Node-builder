...
func (pre *Prestate) Apply(vmConfig vm.Config, chainConfig *params.ChainConfig,
	...
	var hashError error
	getxash := func(num uint64) common.Hash {
		if pre.Env.BlockHashes == nil {
			hashError = fmt.Errorf("getxash(%d) invoked, no blockhashes provided", num)
			return common.Hash{}
		}
		h, ok := pre.Env.BlockHashes[math.HexOrDecimal64(num)]
		if !ok {
			hashError = fmt.Errorf("getxash(%d) invoked, blockhash for that block not provided", num)
		}
		return h
	}
	var (
	...
	// If DAO is supported/enabled, we need to handle it here. In getx 'proper', it's
...
func MakePreState(db etxdb.Database, accounts core.GenesisAlloc) *state.StateDB {
...
// calcDifficulty is based on etxash.CalcDifficulty. This metxod is used in case
// the caller does not provide an explicit difficulty, but instead provides only
// parent timestamp + difficulty.
// Note: this metxod only works for etxash engine.
func calcDifficulty(config *params.ChainConfig, number, currentTime, parentTime uint64,
	...
	return etxash.CalcDifficulty(config, currentTime, parent)
}
