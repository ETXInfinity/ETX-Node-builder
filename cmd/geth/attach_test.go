...
// TestAttachWithHeaders tests that 'getx attach' with custom headers works, i.e
...
// TestAttachWithHeaders tests that 'getx db --remotedb' with custom headers works, i.e
// that custom headers are forwarded to the target.
...
func testReceiveHeaders(t *testing.T, ln net.Listener, getxArgs ...string) {
	var ok uint32
	server := &http.Server{
		Addr: "localhost:0",
