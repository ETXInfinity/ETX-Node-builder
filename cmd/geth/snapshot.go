...
var (
	snapshotCommand = &cli.Command{
		...
		Subcommands: []*cli.Command{
			{
				Name:      "prune-state",
				Usage:     "Prune stale ETX state data based on the snapshot",
				ArgsUsage: "<root>",
				Action:    pruneState,
				...
				Description: `
getx snapshot prune-state <state-root>
...
during the use of Getx, please also specify it here for correct deletion. Otherwise
...
`,
			},
			{
				...
				Description: `
getx snapshot verify-state <state-root>
...
`,
			},
			{
				...
				Flags:     flags.Merge(utils.NetworkFlags, utils.DatabasePathFlags),
				Description: `
getx snapshot check-dangling-storage <state-root> traverses the snap storage 
...
`,
			},
			{
				...
				Flags:     flags.Merge(utils.NetworkFlags, utils.DatabasePathFlags),
				Description: `
getx snapshot inspect-account <address | hash> checks all snapshot layers and prints out
...
`,
			},
			{
				...
				Flags:     flags.Merge(utils.NetworkFlags, utils.DatabasePathFlags),
				Description: `
getx snapshot traverse-state <state-root>
...
`,
			},
			{
				...
				Flags:     flags.Merge(utils.NetworkFlags, utils.DatabasePathFlags),
				Description: `
getx snapshot traverse-rawstate <state-root>
...
`,
			...
			{
				Name:      "dump",
				Usage:     "Dump a specific block from storage (same as 'getx dump' but using snapshots)",
				...
				Description: `
This command is semantically equivalent to 'getx dump', but uses the snapshots
...
func pruneState(ctx *cli.Context) error {
	...
	prunerconfig := pruner.Config{
		...
		Cachedir:  stack.ResolvePath(config.etx.TrieCleanCacheJournal),
