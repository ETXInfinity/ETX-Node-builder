...
func (g *getxrpc) killAndWait() {
	g.getx.Kill()
	g.getx.WaitExit()
}

func (g *getxrpc) callRPC(result interface{}, metxod string, args ...interface{}) {
	if err := g.rpc.Call(&result, metxod, args...); err != nil {
		g.getx.Fatalf("callRPC %v: %v", metxod, err)
	}
}

func (g *getxrpc) addPeer(peer *getxrpc) {
	g.getx.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.getx.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.getx.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.getx.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.getx.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *getxrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func startGetxWithIpc(t *testing.T, name string, args ...string) *getxrpc {
	ipcName := fmt.Sprintf("getx-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	...
	ipcpath := ipcEndpoint(ipcName, g.getx.Datadir)
	// We can't know exactly how long getx will take to start, so we try 10
...
func initGetx(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing getx: %v ", args)
	g := runGetx(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *getxrpc {
	datadir := initGetx(t)
	t.Logf("Importing keys to getx")
	runGetx(t, "account", "import", "--datadir", datadir, "--password", "./testdata/password.txt", "--lightkdf", "./testdata/key.prv").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGetxWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *getxrpc {
	datadir := initGetx(t)
	return startGetxWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
