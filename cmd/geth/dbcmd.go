...
func removeDB(ctx *cli.Context) error {
	...
	path = config.etx.DatabaseFreezer
	switch {
	case path == "":
...
func showLeveldbStats(db etxdb.KeyValueStater) {
...
type snapshotIterator struct {
	init    bool
	account etxdb.Iterator
	storage etxdb.Iterator
}
...
var chainExporters = map[string]func(db etxdb.Database) utils.ChainDataIterator{
	"preimage": func(db etxdb.Database) utils.ChainDataIterator {
		iter := db.NewIterator(rawdb.PreimagePrefix, nil)
		return &preimageIterator{iter: iter}
	},
	"snapshot": func(db etxdb.Database) utils.ChainDataIterator {
...
func showMetaData(ctx *cli.Context) error {
	...
	table.Setxeader([]string{"Field", "Value"})
...
func dbHasLegacyReceipts(db etxdb.Database, firstIdx uint64) (bool, uint64, error) {
	// Check first block for legacy receipt format
	numAncients, err := db.Ancients()
	if err != nil {
		return false, 0, err
