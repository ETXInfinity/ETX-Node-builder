...
`
	...
	{
		getx := runGetx(t, "account", "list", "--datadir", datadir)
		getx.Expect(want)
		getx.ExpectExit()
	}
	{
		getx := runGetx(t, "--datadir", datadir, "account", "list")
		getx.Expect(want)
		getx.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	getx := runGetx(t, "account", "new", "--lightkdf")
	defer getx.ExpectExit()
	getx.Expect(`
...
func TestAccountHelp(t *testing.T) {
	getx := runGetx(t, "account", "-h")
	getx.WaitExit()
	if have, want := getx.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	getx = runGetx(t, "account", "import", "-h")
	getx.WaitExit()
	if have, want := getx.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	getx := runGetx(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer getx.ExpectExit()
	getx.Expect(expected)
}
...
Your new account is locked with a password. Please give a password. Do not forget this password.
!! Unsupported terminal, password will be echoed.
Password: {{.InputLine "sometxing"}}
Repeat password: {{.InputLine "sometxing else"}}
...
func TestWalletImport(t *testing.T) {
	getx := runGetx(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer getx.ExpectExit()
	getx.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(getx.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	getx := runMinimalGetx(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(getx.StderrText(), m) {
...
// https://github.com/ETX/go-ETX/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	getx.SetTemplateFunc("keypath", func(file string) string {
