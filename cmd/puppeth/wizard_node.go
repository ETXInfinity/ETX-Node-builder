...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.etxstats == "" {
		log.Error("No etxstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.etxash != nil && !boot {
		fmt.Println()
		if infos.etxashdir == "" {
			fmt.Printf("Where should the etxash mining DAGs be stored on the remote machine?\n")
			infos.etxashdir = w.readString()
		} else {
			fmt.Printf("Where should the etxash mining DAGs be stored on the remote machine? (default = %s)\n", infos.etxashdir)
			infos.etxashdir = w.readDefaultString(infos.etxashdir)
		}
	}
	...
	if infos.etxstats == "" {
		...
		infos.etxstats = w.readString() + ":" + w.conf.etxstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.etxstats)
		infos.etxstats = w.readDefaultString(infos.etxstats) + ":" + w.conf.etxstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.etxash != nil {
			// etxash based miners only need an etxerbase to mine against
			fmt.Println()
			if infos.etxerbase == "" {
				fmt.Printf("What address should the miner use?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.etxerbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner use? (default = %s)\n", infos.etxerbase)
				infos.etxerbase = w.readDefaultAddress(common.HexToAddress(infos.etxerbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy ETX node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
