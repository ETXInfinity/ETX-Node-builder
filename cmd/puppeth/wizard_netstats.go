...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for etxstats availability")
	if infos, err := checketxstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["etxstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["etxstats"] = infos.Report()
		etxstats = infos.config
	}
...
func (stats serverStats) render() {
	...
	table.Setxeader([]string{"Server", "Address", "Service", "Config", "Value"})
