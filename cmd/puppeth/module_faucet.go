...
FROM ETX/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--etxstats", "{{.etxstats}}", "--etxport", "{{.etxPort}}",     \
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Funding amount (base tier)":   fmt.Sprintf("%d etxers", info.amount),
...
// checkFaucet does a health-check against a faucet server to verify whetxer
