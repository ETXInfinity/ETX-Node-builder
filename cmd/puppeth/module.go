...
// returned from docker inspect, parsed into a form easily usable by puppetx.
type containerInfos struct {
	running bool              // Flag whetxer the container is running currently
...
func inspectContainer(client *sshClient, container string) (*containerInfos, error) {
	// Check whetxer there's a container running for the service
	...
	// Infos retrieved, parse the above into sometxing meaningful
...
// getetxName gets the ETX Name from etxstats
func getetxName(s string) string {
	n := strings.Index(s, ":")
	if n >= 0 {
		return s[:n]
	}
