...
// deployetxstats queries the user for various input on deploying an etxstats
// monitoring server, after which it executes it.
func (w *wizard) deployetxstats() {
	...
	// Retrieve any active etxstats configurations from the server
	infos, err := checketxstats(client, w.network)
	if err != nil {
		infos = &etxstatsInfos{
			port:   80,
			host:   client.server,
	...
	fmt.Println()
	fmt.Printf("Which port should etxstats listen on? (default = %d)\n", infos.port)
	infos.port = w.readDefaultInt(infos.port)

	// Figure which virtual-host to deploy etxstats on
	if infos.host, err = w.ensureVirtualHost(client, infos.port, infos.host); err != nil {
		log.Error("Failed to decide on etxstats host", "err", err)
		return
	}
	// Port and proxy settings retrieved, figure out the secret and boot etxstats
	fmt.Println()
	if infos.secret == "" {
	...
	// Try to deploy the etxstats server on the host
	nocache := false
	if existed {
		fmt.Println()
		fmt.Printf("Should the etxstats be built from scratch (y/n)? (default = no)\n")
	...
	if out, err := deployetxstats(client, w.network, infos.port, infos.secret, infos.host, trusted, infos.banned, nocache); err != nil {
		log.Error("Failed to deploy etxstats container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
