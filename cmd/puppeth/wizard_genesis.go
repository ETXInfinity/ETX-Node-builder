...
func (w *wizard) makeGenesis() {
	...
	fmt.Println(" 1. etxash - proof-of-work")
	fmt.Println(" 2. Clique - proof-of-authority")

	choice := w.read()
	switch {
	case choice == "1":
		// In case of etxash, we're pretty much done
		genesis.Config.etxash = new(params.etxashConfig)
...
// importGenesis imports a Getx genesis spec into puppetx.
...
func (w *wizard) manageGenesis() {
	// Figure out whetxer to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		// Export the native genesis spec used by puppetx and Getx
		getxJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := os.WriteFile(getxJson, out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", getxJson)
