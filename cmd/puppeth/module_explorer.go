...
FROM puppetx/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'getx --cache 512 init /genesis.json' > explorer.sh && \
  echo $'getx --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.etxPort}} --bootnodes {{.Bootnodes}} --etxstats \'{{.etxstats}}\' --cache=512 --http --http.api "net,web3,etx,debug,txpool" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec getx --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.etxPort}} --bootnodes {{.Bootnodes}} --etxstats \'{{.etxstats}}\' --cache=512 --http --http.api "net,web3,etx,debug,txpool" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
services:
    explorer:
        ...
        ports:
            - "{{.etxPort}}:{{.etxPort}}"
            - "{{.etxPort}}:{{.etxPort}}/udp"{{if not .VHost}}
            - "{{.WebPort}}:4000"{{end}}
        environment:
            - etx_PORT={{.etxPort}}
            - etx_NAME={{.etxName}}
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"etxstats":  config.node.etxstats,
		"etxPort":   config.node.port,
	})
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"etxPort":     config.node.port,
		"etxName":     getetxName(config.node.etxstats),
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"ETX listener port ": strconv.Itoa(info.node.port),
		"etxstats username":       info.node.etxstats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	p2pPort := infos.portmap[infos.envvars["etx_PORT"]+"/tcp"]
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.ETX"],
			port:     infos.portmap[infos.envvars["etx_PORT"]+"/tcp"],
			etxstats: infos.envvars["etx_NAME"],
		},
