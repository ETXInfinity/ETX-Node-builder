...
// sshClient is a small wrapper around Go's SSH client with a few utility metxods
// implemented on top.
...
func dial(server string, pubkey []byte) (*sshClient, error) {
	...
	// Configure the supported authentication metxods (ssh agent, private key and password)
	var (
		auths []ssh.AuthMetxod
		conn  net.Conn
	)
...
// capable of acting as puppetx target.
