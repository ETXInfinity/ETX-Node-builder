...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many etxers to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.etxstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.etxstats = w.readString() + ":" + w.conf.etxstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.etxstats)
		infos.node.etxstats = w.readDefaultString(infos.node.etxstats) + ":" + w.conf.etxstats
	}
