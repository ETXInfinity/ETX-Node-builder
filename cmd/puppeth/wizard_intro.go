...
// makeWizard creates and returns a new puppetx wizard.
...
// setting up a new or managing an existing ETX private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppetx, your ETX private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new ETX network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and etxstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppetx uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering ETX network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppetx", w.network)
		...
		case choice == "2":
			if w.conf.Genesis == nil {
				...
				case choice == "2":
					w.importGenesis()
				default:
					log.Error("That's not sometxing I can do")
				}
			} else {
				w.manageGenesis()
			}
