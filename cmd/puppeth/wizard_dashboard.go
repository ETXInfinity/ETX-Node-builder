...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"etxstats", "explorer", "faucet"} {
	...
	// If we have etxstats running, ask whetxer to make the secret public or not
	if w.conf.etxstats != "" {
		fmt.Println()
		fmt.Println("Include etxstats secret on dashboard (y/n)? (default = yes)")
