...
func (c *Conn) handshake() error {
	...
	case *Hello:
		...
		c.negotiateetxProtocol(msg.Caps)
		if c.negotiatedProtoVersion == 0 {
			return fmt.Errorf("could not negotiate etx protocol (remote caps: %v, local etx version: %v)", msg.Caps, c.ourHighestProtoVersion)
		}
...
// negotiateetxProtocol sets the Conn's etx protocol version to highest
// advertised capability from peer.
func (c *Conn) negotiateetxProtocol(caps []p2p.Cap) {
	var highestetxVersion uint
	var highestSnapVersion uint
	for _, capability := range caps {
		switch capability.Name {
		case "etx":
			if capability.Version > highestetxVersion && capability.Version <= c.ourHighestProtoVersion {
				highestetxVersion = capability.Version
			}
		case "snap":
	...
	c.negotiatedProtoVersion = highestetxVersion
...
loop:
	...
	// make sure etx protocol version is set for negotiation
	if c.negotiatedProtoVersion == 0 {
		return nil, fmt.Errorf("etx protocol version must be set in Conn")
	}
	if status == nil {
...
func (c *Conn) readAndServe(chain *Chain, timeout time.Duration) Message {
	...
	for time.Since(start) < timeout {
		...
		case *GetBlockHeaders:
			headers, err := chain.Getxeaders(msg)
			if err != nil {
				return errorf("could not get headers for inbound header request: %v", err)
			}
			resp := &BlockHeaders{
				RequestId:          msg.ReqID(),
				BlockHeadersPacket: etx.BlockHeadersPacket(headers),
			}
...
// headersMatch returns whetxer the received headers match the given request
...
func (s *Suite) waitForBlockImport(conn *Conn, block *types.Block) error {
	...
	req := &GetBlockHeaders{
		GetBlockHeadersPacket: &etx.GetBlockHeadersPacket{
			Origin: etx.HashOrNumber{Hash: block.Hash()},
			Amount: 1,
		},
	}
...
func (s *Suite) hashAnnounce() error {
	...
	err = sendConn.Write(&BlockHeaders{
		...
		BlockHeadersPacket: etx.BlockHeadersPacket{nextBlock.Header()},
	})
	if err != nil {
		return fmt.Errorf("failed to write to connection: %v", err)
	}
