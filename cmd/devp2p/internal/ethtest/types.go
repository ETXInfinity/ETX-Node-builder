...
import (
	...
	"github.com/ETX/go-ETX/p2p/rlpx"
...
type NewPooledTransactionHashes etx.NewPooledTransactionHashesPacket68

func (msg NewPooledTransactionHashes) Code() int     { return 24 }
func (msg NewPooledTransactionHashes) ReqID() uint64 { return 0 }

type GetPooledTransactions etx.GetPooledTransactionsPacket66

func (msg GetPooledTransactions) Code() int     { return 25 }
func (msg GetPooledTransactions) ReqID() uint64 { return msg.RequestId }

type PooledTransactions etx.PooledTransactionsPacket66
...
// Read reads an etx66 packet from the connection.
func (c *Conn) Read() Message {
	...
	case (GetBlockHeaders{}).Code():
		etxMsg := new(etx.GetBlockHeadersPacket66)
		if err := rlp.DecodeBytes(rawData, etxMsg); err != nil {
			return errorf("could not rlp decode message: %v", err)
		}
		return (*GetBlockHeaders)(etxMsg)
	case (BlockHeaders{}).Code():
		etxMsg := new(etx.BlockHeadersPacket66)
		if err := rlp.DecodeBytes(rawData, etxMsg); err != nil {
			return errorf("could not rlp decode message: %v", err)
		}
		return (*BlockHeaders)(etxMsg)
	case (GetBlockBodies{}).Code():
		etxMsg := new(etx.GetBlockBodiesPacket66)
		if err := rlp.DecodeBytes(rawData, etxMsg); err != nil {
			return errorf("could not rlp decode message: %v", err)
		}
		return (*GetBlockBodies)(etxMsg)
	case (BlockBodies{}).Code():
		etxMsg := new(etx.BlockBodiesPacket66)
		if err := rlp.DecodeBytes(rawData, etxMsg); err != nil {
			return errorf("could not rlp decode message: %v", err)
		}
		return (*BlockBodies)(etxMsg)
	case (NewBlock{}).Code():
		msg = new(NewBlock)
	...
	case (NewPooledTransactionHashes66{}).Code():
		// Try decoding to etx68
		etxMsg := new(NewPooledTransactionHashes)
		if err := rlp.DecodeBytes(rawData, etxMsg); err == nil {
			return etxMsg
		}
		msg = new(NewPooledTransactionHashes66)
	case (GetPooledTransactions{}.Code()):
		etxMsg := new(etx.GetPooledTransactionsPacket66)
		if err := rlp.DecodeBytes(rawData, etxMsg); err != nil {
			return errorf("could not rlp decode message: %v", err)
		}
		return (*GetPooledTransactions)(etxMsg)
	case (PooledTransactions{}.Code()):
		etxMsg := new(etx.PooledTransactionsPacket66)
		if err := rlp.DecodeBytes(rawData, etxMsg); err != nil {
			return errorf("could not rlp decode message: %v", err)
		}
		return (*PooledTransactions)(etxMsg)
	default:
...
// Write writes a etx packet to the connection.
