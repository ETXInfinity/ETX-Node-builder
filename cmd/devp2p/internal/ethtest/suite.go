...
import (
	...
	"github.com/ETX/go-ETX/internal/utesting"
...
// NewSuite creates and returns a new etx-test suite that can
...
func (s *Suite) etxTests() []utesting.Test {
...
// a status message with it on the etx protocol.
...
// TestGetBlockHeaders tests whetxer the given node can respond to
// an etx `GetBlockHeaders` request and that the response is accurate.
func (s *Suite) TestGetBlockHeaders(t *utesting.T) {
	...
	// write request
	req := &GetBlockHeaders{
		GetBlockHeadersPacket: &etx.GetBlockHeadersPacket{
			Origin:  etx.HashOrNumber{Hash: s.chain.blocks[1].Hash()},
			Amount:  2,
			Skip:    1,
			Reverse: false,
		},
	...
	expected, err := s.chain.Getxeaders(req)
	if err != nil {
...
func (s *Suite) TestSimultaneousRequests(t *utesting.T) {
	...
	expected1, err := s.chain.Getxeaders(req1)
	if err != nil {
		t.Fatalf("failed to get expected headers for request 1: %v", err)
	}
	expected2, err := s.chain.Getxeaders(req2)
	if err != nil {
...
func (s *Suite) TestSameRequestID(t *utesting.T) {
	...
	expected1, err := s.chain.Getxeaders(request1)
	if err != nil {
		t.Fatalf("failed to get expected block headers: %v", err)
	}
	expected2, err := s.chain.Getxeaders(request2)
	if err != nil {
		t.Fatalf("failed to get expected block headers: %v", err)
	}
	if !headersMatch(expected1, headers1.BlockHeadersPacket) {
...
func (s *Suite) TestZeroRequestID(t *utesting.T) {
	...
	req := &GetBlockHeaders{
		GetBlockHeadersPacket: &etx.GetBlockHeadersPacket{
			Origin: etx.HashOrNumber{Number: 0},
			Amount: 2,
		},
	}
...
// TestGetBlockBodies tests whetxer the given node can respond to
...
func (s *Suite) TestGetBlockBodies(t *utesting.T) {
	...
	req := &GetBlockBodies{
		RequestId: uint64(55),
		GetBlockBodiesPacket: etx.GetBlockBodiesPacket{
...
// TestBroadcast tests whetxer a block announcement is correctly
...
// TestMaliciousTx sends several invalid transactions and tests whetxer
...
// TestLargeTxRequest tests whetxer a node can fulfill a large GetPooledTransactions
// request.
...
// TestNewPooledTxs tests whetxer a node will do a GetPooledTransactions
...
func (s *Suite) TestNewPooledTxs(t *utesting.T) {
	...
	if conn.negotiatedProtoVersion < etx.etx68 {
