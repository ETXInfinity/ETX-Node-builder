...
func TestetxSuite(t *testing.T) {
	getx, err := runGetx()
	if err != nil {
		t.Fatalf("could not run getx: %v", err)
	}
	defer getx.Close()

	suite, err := NewSuite(getx.Server().Self(), fullchainFile, genesisFile)
	if err != nil {
		t.Fatalf("could not create new test suite: %v", err)
	}
	for _, test := range suite.etxTests() {
...
// runGetx creates and starts a getx node
func runGetx() (*node.Node, error) {
...
func setupGetx(stack *node.Node) error {
	chain, err := loadChain(halfchainFile, genesisFile)
	if err != nil {
		return err
	}

	backend, err := etx.New(stack, &etxconfig.Config{
