...
// TestetxProtocolNegotiation tests whetxer the test suite
// can negotiate the highest etx protocol in a status message exchange
func TestetxProtocolNegotiation(t *testing.T) {
	var tests = []struct {
		conn     *Conn
		caps     []p2p.Cap
		expected uint32
	...
	for i, tt := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			tt.conn.negotiateetxProtocol(tt.caps)
			assert.Equal(t, tt.expected, uint32(tt.conn.negotiatedProtoVersion))
		})
	}
}

// TestChain_Getxeaders tests whetxer the test suite can correctly
// respond to a GetBlockHeaders request from a node.
func TestChain_Getxeaders(t *testing.T) {
	...
	var tests = []struct {
	...
	}{
		{
			req: GetBlockHeaders{
				GetBlockHeadersPacket: &etx.GetBlockHeadersPacket{
					Origin:  etx.HashOrNumber{Number: uint64(2)},
					Amount:  uint64(5),
					Skip:    1,
					Reverse: false,
				},
		...
		},
		{
			req: GetBlockHeaders{
				GetBlockHeadersPacket: &etx.GetBlockHeadersPacket{
					Origin:  etx.HashOrNumber{Number: uint64(chain.Len() - 1)},
					Amount:  uint64(3),
					Skip:    0,
					Reverse: true,
				},
		...
		},
		{
			req: GetBlockHeaders{
				GetBlockHeadersPacket: &etx.GetBlockHeadersPacket{
					Origin:  etx.HashOrNumber{Hash: chain.Head().Hash()},
					Amount:  uint64(1),
					Skip:    0,
					Reverse: false,
				},
	...
	for i, tt := range tests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			headers, err := chain.Getxeaders(&tt.req)
			if err != nil {
				t.Fatal(err)
			}
