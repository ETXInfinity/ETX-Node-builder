...
The API-metxod `account_signGnosisSafeTx` was added. This metxod takes two parameters, 
`[address, safeTx]`. The latter, `safeTx`, can be copy-pasted from the gnosis relay. For example: 

```
{
  "jsonrpc": "2.0",
  "metxod": "account_signGnosisSafeTx",
...
Not all fields are required, though. This metxod is really just a UX helper, which massages the 
...
* The external `account_EcRecover`-metxod was reimplemented.
* The external metxod `account_sign(address, data)` was replaced with `account_signData(contentType, address, data)`.
The addition of `contentType` makes it possible to use the metxod for different types of objects, such as:
  * signing data with an intended validator (not yet implemented)
  * signing clique headers,
  * signing plain personal messages,
* The external metxod `account_signTypedData` implements [EIP-712](https://github.com/ETX/EIPs/blob/master/EIPS/eip-712.md) and makes it possible to sign typed data.

#### 4.0.0

* The external `account_Ecrecover`-metxod was removed.
* The external `account_Import`-metxod was removed.

#### 3.0.0

* The external `account_List`-metxod was changed to not expose `url`, which contained info about the local filesystem. It now returns only a list of addresses.

#### 2.0.0

* Commit `73abaf04b1372fa4c43201fb1b8019fe6b0a6f8d`, move `from` into `transaction` object in `signTransaction`. This
makes the `accounts_signTransaction` identical to the old `etx_signTransaction`.
