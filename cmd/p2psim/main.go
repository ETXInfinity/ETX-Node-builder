...
var (
	...
	subscribeFlag = &cli.BoolFlag{
		Name:  "subscribe",
		Usage: "metxod is a subscription",
	}
)
	...
	app.Commands = []*cli.Command{
		...
		},
		{
			...
			Subcommands: []*cli.Command{
				...
				{
					Name:      "rpc",
					ArgsUsage: "<node> <metxod> [<args>]",
					Usage:     "call a node RPC metxod",
					Action:    rpcNode,
					Flags: []cli.Flag{
						subscribeFlag,
					},
...
func rpcNode(ctx *cli.Context) error {
	...
	if ctx.Bool(subscribeFlag.Name) {
		return rpcSubscribe(rpcClient, ctx.App.Writer, metxod, args.Slice()[3:]...)
	}
	...
	if err := rpcClient.Call(&result, metxod, params...); err != nil {
		return err
	}
	return json.NewEncoder(ctx.App.Writer).Encode(result)
}

func rpcSubscribe(client *rpc.Client, out io.Writer, metxod string, args ...string) error {
	parts := strings.SplitN(metxod, "_", 2)
	namespace := parts[0]
	metxod = parts[1]
