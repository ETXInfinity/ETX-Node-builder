...
type GlogHandler struct {
	...
	override  uint32 // Flag whetxer overrides are used, atomically accessible
	backtrace uint32 // Flag whetxer backtrace location is set
...
// Setxandler updates the handler to write records to the specified sub-handler.
func (h *GlogHandler) Setxandler(nh Handler) {
	h.origin = nh
}
...
func (h *GlogHandler) Log(r *Record) error {
	// If backtracing is requested, check whetxer this is the callsite
