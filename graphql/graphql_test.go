...
func TestGraphQLTransactionLogs(t *testing.T) {
	var (
		...
		genesis = &core.Genesis{
			...
			Difficulty: big.NewInt(1048576),
			Alloc: core.GenesisAlloc{
				addr: {Balance: big.NewInt(params.etxer)},
				dad: {
...
func newGQLService(t *testing.T, stack *node.Node, gspec *core.Genesis, genBlocks int, genfunc func(i int, gen *core.BlockGen)) *handler {
	etxConf := &etxconfig.Config{
		Genesis: gspec,
		etxash: etxash.Config{
			PowMode: etxash.ModeFake,
		},
		NetworkId:               1337,
		TrieCleanCache:          5,
	...
	etxBackend, err := etx.New(stack, etxConf)
	if err != nil {
		t.Fatalf("could not create etx backend: %v", err)
	}
	// Create some blocks and import them
	chain, _ := core.GenerateChain(params.AlletxashProtocolChanges, etxBackend.BlockChain().Genesis(),
		etxash.NewFaker(), etxBackend.ChainDb(), genBlocks, genfunc)
	_, err = etxBackend.BlockChain().InsertChain(chain)
	if err != nil {
		t.Fatalf("could not create import blocks: %v", err)
	}
	// Set up handler
	filterSystem := filters.NewFilterSystem(etxBackend.APIBackend, filters.Config{})
	handler, err := newHandler(stack, etxBackend.APIBackend, filterSystem, []string{}, []string{})
	if err != nil {
