...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte ETX address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an ETX account at a particular block.
    type Account {
    ...
    # Transaction is an ETX transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an ETX account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an ETX account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an ETX block by number or by hash. If neither is
