...
func stackTrieFromPool(db etxdb.KeyValueWriter, owner common.Hash) *StackTrie {
...
type StackTrie struct {
	...
	db       etxdb.KeyValueWriter // Pointer to the commit db, can be nil
}

// NewStackTrie allocates and initializes an empty trie.
func NewStackTrie(db etxdb.KeyValueWriter) *StackTrie {
	return &StackTrie{
		nodeType: emptyNode,
		db:       db,
	}
...
func NewStackTrieWithOwner(db etxdb.KeyValueWriter, owner common.Hash) *StackTrie {
	return &StackTrie{
		owner:    owner,
		nodeType: emptyNode,
		db:       db,
...
func NewFromBinary(data []byte, db etxdb.KeyValueWriter) (*StackTrie, error) {
	var st StackTrie
...
func (st *StackTrie) setDb(db etxdb.KeyValueWriter) {
	st.db = db
	for _, child := range st.children {
		if child != nil {
			child.setDb(db)
...
func newLeaf(owner common.Hash, key, val []byte, db etxdb.KeyValueWriter) *StackTrie {
...
func newExt(owner common.Hash, key []byte, child *StackTrie, db etxdb.KeyValueWriter) *StackTrie {
	st := stackTrieFromPool(db, owner)
	st.nodeType = extNode
	st.key = append(st.key, key...)
	st.children[0] = child
...
//
// This metxod also sets 'st.type' to hashedNode, and clears 'st.key'.
