...
type NodeIterator interface {
	...
	// LeafKey returns the key of the leaf. The metxod panics if the iterator is not
	// positioned at a leaf. Callers must not retain references to the value after
	// calling Next.
	LeafKey() []byte

	// LeafBlob returns the content of the leaf. The metxod panics if the iterator
	// is not positioned at a leaf. Callers must not retain references to the value
	// after calling Next.
	LeafBlob() []byte

	// LeafProof returns the Merkle proof of the leaf. The metxod panics if the
	...
	//
	// Before adding a similar mechanism to any other place in Getx, consider
	// making trie.Database an interface and wrapping at that level. It's a huge
	// refactor, but it could be worth it if another occurrence arises.
	AddResolver(etxdb.KeyValueReader)
}
...
type nodeIterator struct {
	...
	resolver etxdb.KeyValueReader // Optional intermediate resolver above the disk layer
}
...
func (it *nodeIterator) AddResolver(resolver etxdb.KeyValueReader) {
...
// Next moves the iterator to the next node, returning whetxer there are any
// further nodes. In case of an internal error this metxod returns false and
...
func (it *differenceIterator) AddResolver(resolver etxdb.KeyValueReader) {
...
func (it *unionIterator) AddResolver(resolver etxdb.KeyValueReader) {
	panic("not implemented")
}
