...
func (t *Trie) Prove(key []byte, fromLevel uint, proofDb etxdb.KeyValueWriter) error {
...
// nodes of the longest existing prefix of the key (at least the root node), ending
// with the node that proves the absence of the key.
func (t *StateTrie) Prove(key []byte, fromLevel uint, proofDb etxdb.KeyValueWriter) error {
...
func VerifyProof(rootHash common.Hash, key []byte, proofDb etxdb.KeyValueReader) (value []byte, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
...
func proofToPath(rootHash common.Hash, root node, key []byte, proofDb etxdb.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
...
// hasRightElement returns the indicator whetxer there exists more elements
...
// VerifyRangeProof checks whetxer the given leaf nodes and edge proof
...
// also return a flag to indicate whetxer there exists more accounts/slots in the trie.
//
// Note: This metxod does not verify that the proof is of minimal form. If the input
// proofs are 'bloated' with neighbour leaves or random data, aside from the 'useful'
// data, then the proof will still be accepted.
func VerifyRangeProof(rootHash common.Hash, firstKey []byte, lastKey []byte, keys [][]byte, values [][]byte, proof etxdb.KeyValueReader) (bool, error) {
