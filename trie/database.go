...
type Database struct {
	diskdb etxdb.KeyValueStore // Persistent storage for matured trie nodes
...
type Config struct {
	...
	Preimages bool   // Flag whetxer the preimage of trie key is recorded
}

// NewDatabase creates a new trie database to store ephemeral trie content before
// its written out to disk or garbage collected. No read cache is created, so all
// data retrievals will hit the underlying disk database.
func NewDatabase(diskdb etxdb.KeyValueStore) *Database {
...
func NewDatabaseWithConfig(diskdb etxdb.KeyValueStore, config *Config) *Database {
...
// cached, the metxod queries the persistent database for the content.
...
// This metxod is extremely expensive and should only be used to validate internal
// states in test code.
...
// are referenced togetxer by database itself.
...
//
// Note, this metxod is a non-synchronized mutator. It is unsafe to call this
...
func (db *Database) commit(hash common.Hash, batch etxdb.Batch, uncacher *cleaner, callback func(common.Hash)) error {
