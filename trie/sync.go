...
type Sync struct {
	database etxdb.KeyValueReader         // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database etxdb.KeyValueReader, callback LeafCallback) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) AddSubTrie(root common.Hash, path []byte, parent common.Hash, parentPath []byte, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them togetxer
...
// both etx/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw etxdb.Batch) error {
...
func (s *Sync) children(req *nodeRequest, object node) ([]*nodeRequest, error) {
	// Gather all the children of the node, irrelevant whetxer known or not
