...
type serverHandler struct {
	...
	synced  func() bool    // Callback function used to determine whetxer local node is synced.

	// Testing fields
	addTxsSync bool
}

func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb etxdb.Database, txpool *txpool.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	p.Log().Debug("Light ETX peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, h.blockchain.Genesis().Hash(), forkID, h.forkFilter, h.server); err != nil {
		p.Log().Debug("Light ETX handshake failed", "err", err)
		return err
	}
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light ETX response", "err", err)
			return err
		default:
		}
		if err := h.handleMsg(p, &wg); err != nil {
			p.Log().Debug("Light ETX message handling failed", "err", err)
			return err
		}
	}
}
...
func (h *serverHandler) handleMsg(p *clientPeer, wg *sync.WaitGroup) error {
	...
	p.Log().Trace("Light ETX message arrived", "code", msg.Code, "bytes", msg.Size)
...
// GetxelperTrie returns the post-processed trie root for the given trie ID and section index
func (h *serverHandler) GetxelperTrie(typ uint, index uint64) *trie.Trie {
	var (
		root   common.Hash
		prefix string
	)
...
func (h *serverHandler) broadcastLoop() {
	...
	for {
		...
		case ev := <-headCh:
			...
			var reorg uint64
			if lastHead != nil {
				// If a setxead has been performed, the common ancestor can be nil.
