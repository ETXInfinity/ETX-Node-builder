...
func generatePreMergeChain(n int) (*core.Genesis, []*types.Header, []*types.Block) {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(genesis, etxash.NewFaker(), n, nil)
...
func startLesService(t *testing.T, genesis *core.Genesis, headers []*types.Header) (*node.Node, *les.LightETX) {
	t.Helper()
	...
	etxcfg := &etxconfig.Config{
		Genesis:        genesis,
		etxash:         etxash.Config{PowMode: etxash.ModeFake},
		SyncMode:       downloader.LightSync,
		TrieDirtyCache: 256,
		TrieCleanCache: 256,
		LightPeers:     10,
	}
	lesService, err := les.New(n, etxcfg)
	if err != nil {
