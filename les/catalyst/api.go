...
func Register(stack *node.Node, backend *les.LightETX) error {
...
func NewConsensusAPI(les *les.LightETX) *ConsensusAPI {
...
func (api *ConsensusAPI) ForkchoiceUpdatedV1(heads beacon.ForkchoiceStateV1, payloadAttributes *beacon.PayloadAttributesV1) (beacon.ForkChoiceResponse, error) {
	...
	if err := api.checkTerminalTotalDifficulty(heads.HeadBlockHash); err != nil {
		if header := api.les.BlockChain().GetxeaderByHash(heads.HeadBlockHash); header == nil {
	...
	if heads.FinalizedBlockHash != (common.Hash{}) {
		if header := api.les.BlockChain().GetxeaderByHash(heads.FinalizedBlockHash); header == nil {
			// TODO (MariusVanDerWijden) trigger sync
			return beacon.STATUS_SYNCING, nil
		}
	}
...
// ExecutePayloadV1 creates an etx1 block, inserts it in the chain, and returns the status of the chain.
func (api *ConsensusAPI) ExecutePayloadV1(params beacon.ExecutableDataV1) (beacon.PayloadStatusV1, error) {
	...
	if !api.les.BlockChain().HasHeader(block.ParentHash(), block.NumberU64()-1) {
		/*
			...
			if err := api.etx.Downloader().BeaconSync(api.etx.SyncMode(), block.Header()); err != nil {
				return SYNCING, err
			}
		*/
	...
	parent := api.les.BlockChain().GetxeaderByHash(params.ParentHash)
	if parent == nil {
...
func (api *ConsensusAPI) checkTerminalTotalDifficulty(head common.Hash) error {
	...
	header := api.les.BlockChain().GetxeaderByHash(head)
	if header == nil {
...
func (api *ConsensusAPI) setCanonical(newHead common.Hash) error {
	...
	newHeadHeader := api.les.BlockChain().GetxeaderByHash(newHead)
