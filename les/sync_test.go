...
func testMissOracleBackend(t *testing.T, hasCheckpoint bool, protocol int) {
	...
	// that user wants to unlock sometxing which blocks the oracle backend
	// initialisation. But at the same time syncing starts.
	//
	// See https://github.com/ETX/go-ETX/issues/20097 for more detail.
	//
...
func testSyncFromConfiguredCheckpoint(t *testing.T, protocol int) {
	...
	head := server.handler.blockchain.GetxeaderByNumber(config.ChtSize - 1).Hash()
	cp := &params.TrustedCheckpoint{
		SectionIndex: 0,
		SectionHead:  head,
