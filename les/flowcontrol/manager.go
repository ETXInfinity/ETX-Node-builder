...
var (
	...
	capacityRaisetxresholdRatio = 1.125                        // total/connected capacity ratio threshold for raising the capacity factor
)
...
// than this threshold or if their ratio is less than capacityRaisetxresholdRatio
// then capFactor is allowed to slowly raise.
func (cm *ClientManager) SetCapacityLimits(min, max, raisetxreshold uint64) {
	if min < 1 {
		min = 1
	}
	...
	cm.capacityRaisetxreshold = raisetxreshold
	cm.refreshCapacity()
}
...
func (cm *ClientManager) updateRaiseLimit() {
	if cm.capacityRaisetxreshold == 0 {
		cm.logTotalCapRaiseLimit = 0
		return
	}
	limit := float64(cm.totalConnected + cm.capacityRaisetxreshold)
	limit2 := float64(cm.totalConnected) * capacityRaisetxresholdRatio
	if limit2 > limit {
		limit = limit2
	}
	if limit < 1 {
...
func (cm *ClientManager) updateRecharge(now mclock.AbsTime) {
	...
	for cm.sumRecharge > 0 {
		...
		// check whetxer it has already finished
