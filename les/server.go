...
type LesServer struct {
	...
	archiveMode bool // Flag whetxer the ETX node runs in archive mode.
...
func NewLesServer(node *node.Node, e etxBackend, config *etxconfig.Config) (*LesServer, error) {
	lesDb, err := node.OpenDatabase("les.server", 0, 0, "etx/db/lesserver/", false)
	if err != nil {
		return nil, err
	}
	...
	srv.vfluxServer.Register(srv.clientPool, "les", "ETX light client service")
