...
type nodeDB struct {
	...
	evictCallBack func(mclock.AbsTime, bool, utils.ExpiredValue) bool // Callback to determine whetxer the balance can be evicted.
	clock         mclock.Clock
	closeCh       chan struct{}
	cleanupHook   func() // Test hook used for testing
}

func newNodeDB(db etxdb.KeyValueStore, clock mclock.Clock) *nodeDB {
	ndb := &nodeDB{
		db:      db,
...
// expireNodes iterates the whole node db and checks whetxer the
