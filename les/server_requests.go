...
type serverBackend interface {
	...
	GetxelperTrie(typ uint, index uint64) *trie.Trie
}
...
var Les3 = map[uint64]RequestType{
	...
	GetxelperTrieProofsMsg: {
		...
		Handle:           handleGetxelperTrieProofs,
	},
	SendTxV2Msg: {
...
func handleGetBlockHeaders(msg Decoder) (serveRequestFn, uint64, uint64, error) {
	...
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		for !unknown && len(headers) < int(r.Query.Amount) && bytes < softResponseLimit {
			...
			if hashMode {
				if first {
					origin = bc.GetxeaderByHash(r.Query.Origin.Hash)
					if origin != nil {
						r.Query.Origin.Number = origin.Number.Uint64()
					}
				} else {
					origin = bc.Getxeader(r.Query.Origin.Hash, r.Query.Origin.Number)
				}
			} else {
				origin = bc.GetxeaderByNumber(r.Query.Origin.Number)
			}
			if origin == nil {
				break
			}
			...
			case hashMode && !r.Query.Reverse:
				...
				if next <= current {
					...
					if header := bc.GetxeaderByNumber(next); header != nil {
...
func handleGetCode(msg Decoder) (serveRequestFn, uint64, uint64, error) {
	...
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		for i, request := range r.Reqs {
			...
			header := bc.GetxeaderByHash(request.BHash)
			if header == nil {
...
func handleGetReceipts(msg Decoder) (serveRequestFn, uint64, uint64, error) {
	...
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		for i, hash := range r.Hashes {
			...
			if results == nil {
				if header := bc.GetxeaderByHash(hash); header == nil || header.ReceiptHash != types.EmptyRootHash {
					p.bumpInvalid()
					continue
				}
			}
	...
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		for i, request := range r.Reqs {
			...
			if request.BHash != lastBHash {
				...
				if header = bc.GetxeaderByHash(request.BHash); header == nil {
...
// handleGetxelperTrieProofs handles a helper trie proof request
func handleGetxelperTrieProofs(msg Decoder) (serveRequestFn, uint64, uint64, error) {
	var r GetxelperTrieProofsPacket
	if err := msg.Decode(&r); err != nil {
		return nil, 0, 0, err
	}
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		for i, request := range r.Reqs {
			...
			if auxTrie == nil || request.Type != lastType || request.TrieIdx != lastIdx {
				...
				auxTrie = backend.GetxelperTrie(request.Type, request.TrieIdx)
			}
			if auxTrie == nil {
				return nil
			}
			...
			if request.Type == htCanonical && request.AuxReq == htAuxHeader && len(request.Key) == 8 {
				header := bc.GetxeaderByNumber(binary.BigEndian.Uint64(request.Key))
