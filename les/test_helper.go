...
import (
	...
	"github.com/ETX/go-ETX/accounts/abi/bind/backends"
	...
	"github.com/ETX/go-ETX/contracts/checkpointoracle/contract"
	...
	"github.com/ETX/go-ETX/les/checkpointoracle"
	"github.com/ETX/go-ETX/les/flowcontrol"
	vfs "github.com/ETX/go-ETX/les/vflux/server"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some etxer to user1
		...
		case 1:
			...
			// bankUser transfers more etxer to user1
		...
		case 2:
			...
			// bankUser transfer some etxer to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db etxdb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db etxdb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) (*clientHandler, func()) {
	var (
	...
	client := &LightETX{
		lesCommons: lesCommons{
			...
			config:      &etxconfig.Config{LightPeers: 100, NetworkId: NetworkId},
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db etxdb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend, func()) {
	var (
		gspec = core.Genesis{
			Config:   params.AlletxashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
