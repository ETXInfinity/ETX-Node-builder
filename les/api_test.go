...
import (
	...
	etxdownloader "github.com/ETX/go-ETX/etx/downloader"
	...
	"github.com/ETX/go-ETX/p2p/simulations"
	"github.com/ETX/go-ETX/p2p/simulations/adapters"
...
func TestMain(m *testing.M) {
	...
	log.Root().Setxandler(log.LvlFilterHandler(log.Lvl(*loglevel), log.StreamHandler(colorable.NewColorableStderr(), log.TerminalFormat(true))))
...
// similar requests at the maximum allowed rate and the test verifies whetxer the
...
func testCapacityAPI(t *testing.T, clientCount int) {
	...
	for !testSim(t, 1, clientCount, []string{testServerDataDir}, nil, func(ctx context.Context, net *simulations.Network, servers []*simulations.Node, clients []*simulations.Node) bool {
		...
		headNum, headHash := getxead(ctx, t, serverRpcClient)
		...
		for i, client := range clients {
			...
			for {
				...
				num, hash := getxead(ctx, t, clientRpcClients[i])
...
func getxead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	res := make(map[string]interface{})
	if err := client.CallContext(ctx, &res, "etx_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	defer cancel()
	err := client.CallContext(c, &res, "etx_getBalance", addr, "latest")
	if err != nil {
...
func newLesClientService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	config := etxconfig.Defaults
	config.SyncMode = (etxdownloader.SyncMode)(downloader.LightSync)
	config.etxash.PowMode = etxash.ModeFake
	return New(stack, &config)
}

func newLesServerService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	config := etxconfig.Defaults
	config.SyncMode = (etxdownloader.SyncMode)(downloader.FullSync)
	config.LightServ = testServerCapacity
	config.LightPeers = testMaxClients
	ETX, err := etx.New(stack, &config)
	if err != nil {
		return nil, err
	}
	_, err = NewLesServer(stack, ETX, &config)
	if err != nil {
		return nil, err
	}
	return ETX, nil
