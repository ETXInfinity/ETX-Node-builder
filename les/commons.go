...
func (c *lesCommons) setupOracle(node *node.Node, genesis common.Hash, etxconfig *etxconfig.Config) *checkpointoracle.CheckpointOracle {
	config := etxconfig.CheckpointOracle
	if config == nil {
	...
	client := etxclient.NewClient(rpcClient)
	oracle.Start(client)
