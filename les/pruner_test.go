...
func TestLightPruner(t *testing.T) {
	...
	var cases = []struct {
		from, to   uint64
		metxodName string
		metxod     func(uint64) bool
	}{
		{
			1, 10, "GetxeaderByNumber",
			func(n uint64) bool {
				_, err := light.GetxeaderByNumber(context.Background(), client.handler.backend.odr, n)
				return err == nil
			},
		},
		{
		...
		},
		{
			21, 30, "GetTd",
			func(n uint64) bool {
				_, err := light.GetTd(context.Background(), client.handler.backend.odr, server.handler.blockchain.GetxeaderByNumber(n).Hash(), n)
				return err == nil
			},
		},
		{
			31, 40, "GetBodyRLP",
			func(n uint64) bool {
				_, err := light.GetBodyRLP(context.Background(), client.handler.backend.odr, server.handler.blockchain.GetxeaderByNumber(n).Hash(), n)
				return err == nil
			},
		},
		{
			41, 50, "GetBlock",
			func(n uint64) bool {
				_, err := light.GetBlock(context.Background(), client.handler.backend.odr, server.handler.blockchain.GetxeaderByNumber(n).Hash(), n)
				return err == nil
			},
		},
		{
			...
			func(n uint64) bool {
				_, err := light.GetBlockReceipts(context.Background(), client.handler.backend.odr, server.handler.blockchain.GetxeaderByNumber(n).Hash(), n)
				return err == nil
			},
		},
	}
	for _, c := range cases {
		for i := c.from; i <= c.to; i++ {
			if !c.metxod(i) {
				t.Fatalf("rpc metxod %s failed, number %d", c.metxodName, i)
			}
		}
	}
	// Check GetBloombits
