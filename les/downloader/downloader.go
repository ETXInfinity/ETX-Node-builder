...
// Package downloader is a temporary package whilst working on the etx/66 blocking refactors.
...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [etx/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB etxdb.Database // Database to state sync into (and deduplicate via)
	...
	snapSync       bool         // Whetxer to run state sync over the snap protocol
	...
	// Testing hooks
	syncInitHook     func(uint64, uint64)  // Metxod to call upon initiating a new sync run
	bodyFetchHook    func([]*types.Header) // Metxod to call upon starting a block body fetch
	receiptFetchHook func([]*types.Header) // Metxod to call upon starting a receipt fetch
	chainInsertHook  func([]*fetchResult)  // Metxod to call upon inserting a chain of blocks (possibly in multiple invocations)
}
...
type LightChain interface {
	...
	// GetxeaderByHash retrieves a header from the local chain.
	GetxeaderByHash(common.Hash) *types.Header
...
func New(checkpoint uint64, stateDb etxdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() ETX.SyncProgress {
...
// Synchronising returns whetxer the downloader is currently retrieving blocks.
...
func (d *Downloader) Synchronise(id string, head common.Hash, td *big.Int, mode SyncMode) error {
	...
	if errors.Is(err, errInvalidChain) || errors.Is(err, errBadPeer) || errors.Is(err, errTimeout) ||
		...
		if d.dropPeer == nil {
			// The dropPeer metxod is nil when `--copydb` is used for a local copy.
...
// checks fail an error will be returned. This metxod is synchronous
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode) error {
	...
	// sync mode. Long term we could drop fast sync or merge the two togetxer,
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	if p.version < etx.etx66 {
		return fmt.Errorf("%w: advertized %d < required %d", errTooOld, p.version, etx.etx66)
	}
	mode := d.getMode()

	log.Debug("Synchronising with the network", "peer", p.id, "etx", p.version, "head", hash, "td", td, "mode", mode)
	...
	if mode == FastSync {
		...
		if origin+1 < frozen {
			if err := d.lightchain.Setxead(origin + 1); err != nil {
				return err
			}
		}
	}
...
// not wait for the running download goroutines to finish. This metxod should be
	...
	if mode == LightSync {
		...
		if d.genesis == 0 {
			...
			for header != nil {
				...
				header = d.lightchain.GetxeaderByHash(header.ParentHash)
			}
		}
...
func (d *Downloader) findAncestorSpanSearch(p *peerConnection, mode SyncMode, remoteHeight, localHeight uint64, floor int64) (commonAncestor uint64, err error) {
	...
	for finished := false; !finished; {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave sometxing valid
			headers := packet.(*headerPack).headers
			if len(headers) == 0 {
	...
	for start+1 < end {
		...
		for arrived := false; !arrived; {
			...
			case packet := <-d.headerCh:
				...
				header := d.lightchain.GetxeaderByHash(h) // Independent of sync mode, header surely exists
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64) error {
	...
	pivoting := false           // Whetxer the next request is pivot verification
...
//
// The metxod returns the entire filled skeleton and also the number of headers
...
func (d *Downloader) fillHeaderSkeleton(from uint64, skeleton []*types.Header) ([]*types.Header, int, error) {
	...
	var (
		...
		setIdle  = func(p *peerConnection, accepted int, deliveryTime time.Time) {
			p.SetxeadersIdle(accepted, deliveryTime)
		}
	)
...
// types, this metxod is used by each for data gathering and is instrumented with
...
//   - expire:      task callback metxod to abort requests that took too long and return the faulty peers (traffic shaping)
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered sometxing completely else than requested (usually
...
func (d *Downloader) processHeaders(origin uint64, td *big.Int) error {
	...
	defer func() {
		if rollback > 0 {
			...
			if err := d.lightchain.Setxead(rollback - 1); err != nil { // -1 to target the parent of the first uncertain block
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have sometxing)
				// R: Nothing to give
				if mode != LightSync {
				...
				// peer gave us sometxing useful, we're already happy/progressed (above check).
			...
			gotHeaders = true
			for len(headers) > 0 {
				// Terminate if sometxing failed in between processing chunks
				select {
				case <-d.cancelCh:
					rollbackErr = errCanceled
					return errCanceled
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The InsertChain metxod in blockchain.go will sometimes return an out-of-bounds index,
			// when it needs to preprocess blocks to import a sidechain.
			// The importer will put togetxer a new list of blocks to import, which is a superset
