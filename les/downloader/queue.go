...
type fetchRequest struct {
	...
	From    uint64          // [etx/62] Requested chain element index (used for skeleton fills only)
	Headers []*types.Header // [etx/62] Requested headers, sorted by request order
...
func (q *queue) Reset(blockCacheLimit int, thresholdInitialSize int) {
	...
	q.resultCache.SetThrottletxreshold(uint64(thresholdInitialSize))
}
...
// InFlightHeaders retrieves whetxer there are header fetch requests currently
// in flight.
...
// InFlightBlocks retrieves whetxer there are block fetch requests currently in
// flight.
...
// InFlightReceipts retrieves whetxer there are receipt fetch requests currently
// in flight.
...
func (q *queue) Results(block bool) []*fetchResult {
	...
	// on the result cache
	throttletxreshold := uint64((common.StorageSize(blockCacheMemory) + q.resultSize - 1) / q.resultSize)
	throttletxreshold = q.resultCache.SetThrottletxreshold(throttletxreshold)

	// Log some info at certain times
	if time.Since(q.lastStatLog) > 60*time.Second {
		q.lastStatLog = time.Now()
		info := q.Stats()
		info = append(info, "throttle", throttletxreshold)
...
func (q *queue) ReserveHeaders(p *peerConnection, count int) *fetchRequest {
	...
	// Short circuit if the peer's already downloading sometxing (sanity check to
	// not corrupt state)
...
// returns a flag whetxer empty blocks were queued requiring processing.
...
// also returns a flag whetxer empty receipts were queued requiring importing.
...
// skipping any previously failed ones. This metxod is a generic version used
// by the individual special reservation functions.
//
// Note, this metxod expects the queue lock to be already held for writing. The
...
//	progress - whetxer any progress was made
//	throttle - if the caller should throttle for a while
func (q *queue) reserveHeaders(p *peerConnection, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	pendPool map[string]*fetchRequest, kind uint) (*fetchRequest, bool, bool) {
	// Short circuit if the pool has been depleted, or if the peer's already
	// downloading sometxing (sanity check not to corrupt state)
	if taskQueue.Empty() {
...
// Revoke cancels all pending requests belonging to a given peer. This metxod is
...
//
// Note, this metxod expects the queue lock to be already held. The
// reason the lock is not obtained in here is because the parameters already need
// to access the queue, so they already need a lock anyway.
...
// cache. This metxod either accepts all headers it received, or none of them
// if they do not map correctly to the skeleton.
//
// If the headers are accepted, the metxod makes an attempt to deliver the set
...
// The metxod returns the number of blocks bodies accepted from the delivery and
...
// The metxod returns the number of transaction receipts accepted from the delivery
