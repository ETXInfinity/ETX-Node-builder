...
type resultStore struct {
	...
	// throttletxreshold is the limit up to which we _want_ to fill the
...
func newResultStore(size int) *resultStore {
	return &resultStore{
		...
		throttletxreshold: uint64(size),
	}
}

// SetThrottletxreshold updates the throttling threshold based on the requested
// limit and the total queue capacity. It returns the (possibly capped) threshold
func (r *resultStore) SetThrottletxreshold(threshold uint64) uint64 {
	r.lock.Lock()
	defer r.lock.Unlock()
	...
	r.throttletxreshold = threshold
	return r.throttletxreshold
}
...
// is true, that means the header has already been delivered 'upstream'. This metxod
...
func (r *resultStore) getFetchResult(headerNumber uint64) (item *fetchResult, index int, stale, throttle bool, err error) {
	...
	throttle = index >= int(r.throttletxreshold)
	stale = index < 0
...
// this metxod is cheaper than countCompleted
...
//
// The metxod assumes (at least) rlock is held.
