...
func newClientHandler(ulcServers []string, ulcFraction int, checkpoint *params.TrustedCheckpoint, backend *LightETX) *clientHandler {
...
func (h *clientHandler) handle(p *serverPeer, noInitAnnounce bool) error {
	...
	if err := h.backend.peers.register(p); err != nil {
		p.Log().Error("Light ETX peer registration failed", "err", err)
		return err
	}
...
func (d *downloaderPeerNotify) registerPeer(p *serverPeer) {
	...
	h.downloader.RegisterLightPeer(p.id, etx.etx66, pc)
}
