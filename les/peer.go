...
import (
	...
	"github.com/ETX/go-ETX/les/utils"
...
type peerCommons struct {
	*p2p.Peer
	...
	frozen       uint32    // Flag whetxer the peer is frozen.
...
// canQueue returns an indicator whetxer the peer can queue an operation.
...
// PeerInfo represents a short summary of the `etx` sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // ETX protocol version negotiated
...
func (p *peerCommons) handshake(td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter, sendCallback func(*keyValueList), recvCallback func(keyValueMap) error) error {
	...
	// as well. Check http://eips.ETX.org/EIPS/eip-2124 for more
	// spec detail.
	if p.version >= lpv4 {
...
type serverPeer struct {
	...
	// Status fields
	trusted                 bool   // The flag whetxer the server is selected as trusted server.
	onlyAnnounce            bool   // The flag whetxer the server sends announcement only.
	...
	// Test callback hooks
	hasBlockHook func(common.Hash, uint64, bool) bool // Used to determine whetxer the server has the specified block.
}
...
func (p *serverPeer) requestHelperTrieProofs(reqID uint64, reqs []HelperTrieReq) error {
	...
	return p.sendRequest(GetxelperTrieProofsMsg, reqID, reqs, len(reqs))
}
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local ETX node is running in archive mode, advertise ourselves we have
...
// participating in the Light ETX sub-protocol.
...
// for us(server) but for other protocols(e.g. etx) upon the devp2p they
// may be useful.
