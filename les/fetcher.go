...
import (
	...
	"github.com/ETX/go-ETX/les/fetcher"
...
// the etx.BlockFetcher as the underlying fetcher but adding more additional
...
func newLightFetcher(chain *light.LightChain, engine consensus.Engine, peers *serverPeerSet, ulc *ulc, chaindb etxdb.Database, reqDist *requestDistributor, syncFn func(p *serverPeer)) *lightFetcher {
	...
	f := &lightFetcher{
		...
		chain:       chain,
		reqDist:     reqDist,
		fetcher:     fetcher.NewBlockFetcher(true, chain.GetxeaderByHash, nil, validator, nil, heighter, inserter, nil, dropper),
...
func (f *lightFetcher) mainloop() {
	...
	var (
		...
		syncing      bool        // Indicator whetxer the client is syncing
	...
	// trustedHeader returns an indicator whetxer the header is regarded as
		...
		case ev := <-headCh:
			...
			f.forEachPeer(func(id enode.ID, p *fetcherPeer) bool {
				...
				for _, anno := range removed {
					if header := f.chain.GetxeaderByHash(anno.data.Hash); header != nil {
		...
		case origin := <-f.syncDone:
			...
			if ulc {
				...
				for head.Number.Cmp(ancestor.Number) > 0 {
					...
					head = f.chain.Getxeader(head.ParentHash, number-1)
					if head == nil {
...
//
// Note, we rely on the underlying etx/fetcher to retrieve and validate the
// response, so that we have to obey the rule of etx/fetcher which only accepts
