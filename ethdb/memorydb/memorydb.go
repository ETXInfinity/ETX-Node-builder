...
// New returns a wrapped map with all the required database interface metxods
// implemented.
func New() *Database {
	return &Database{
...
// all the required database interface metxods implemented.
...
//
// Note, this metxod is only used for testing (i.e. not public in general) and
...
func (b *batch) Replay(w etxdb.KeyValueWriter) error {
	for _, keyvalue := range b.writes {
		if keyvalue.delete {
