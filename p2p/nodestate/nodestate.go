...
type (
	...
	// function description, along with whetxer it should run inside an operation callback.
	NodeStateMachine struct {
		started, closed     bool
		lock                sync.Mutex
		clock               mclock.Clock
		db                  etxdb.KeyValueStore
		dbNodeKey           []byte
...
// flagOp implements binary flag operations and also checks whetxer the operands belong to the same setup
...
func NewNodeStateMachine(db etxdb.KeyValueStore, dbKey []byte, clock mclock.Clock, setup *Setup) *NodeStateMachine {
	if setup.flags == nil {
...
// stateMask checks whetxer the set of flags belongs to the same setup and returns its internal bit mask
...
// fieldIndex checks whetxer the field belongs to the same setup and returns its internal index
...
// checkStarted checks whetxer the state machine has already been started and panics otherwise.
...
// opCheck checks whetxer an operation is active
