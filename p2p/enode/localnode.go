...
const (
	...
	recordUpdatetxrottle = time.Millisecond
)

// LocalNode produces the signed node record of a local node, i.e. a node run in the
// current process. Setting ENR entries via the Set metxod updates the record. A new version
// of the record is signed on demand when the Node metxod is called.
...
func (ln *LocalNode) Node() *Node {
	...
	if lastChange < recordUpdatetxrottle {
		time.Sleep(recordUpdatetxrottle - lastChange)
	}
