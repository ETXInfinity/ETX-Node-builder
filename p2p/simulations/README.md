...
network has exposed metxods for creating, starting, stopping, connecting 
...
which is the outcome of sometxing happening in the network as a result of a
...
With a running simulation network, the `Simulation.Run` metxod can be called
with a `Step` which has the following fields:

* `Action` - a function that performs some action in the network

* `Expect` - an expectation function which returns whetxer or not a
...
As a concrete example, consider a simulated network of ETX nodes. An
...
On return, the `Simulation.Run` metxod returns a `StepResult` which can be used
...
p2psim node rpc <node> <metxod> [<args>] [--subscribe]
```
