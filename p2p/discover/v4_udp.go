...
import (
	...
	"github.com/ETX/go-ETX/p2p/discover/v4wire"
...
const (
	...
	ntpFailuretxreshold = 32               // Continuous timeouts after which to check NTP
...
type replyMatcher struct {
	...
	// reply was acceptable. The second return value indicates whetxer the callback should
...
type reply struct {
	...
	ip   net.IP
	data v4wire.Packet
	// loop indicates whetxer there was
...
// whetxer any matcher considered the packet acceptable.
...
func (t *UDPv4) loop() {
	...
	for {
		...
		case r := <-t.gotreply:
			var matched bool // whetxer any replyMatcher considered the reply acceptable.
			for el := plist.Front(); el != nil; el = el.Next() {
		...
		case now := <-timeout.C:
			...
			if contTimeouts > ntpFailuretxreshold {
...
func (t *UDPv4) wrapPacket(p v4wire.Packet) *packetxandlerV4 {
	var h packetxandlerV4
	h.Packet = p
	switch p.(type) {
	case *v4wire.Ping:
...
// packetxandlerV4 wraps a packet with handler functions.
type packetxandlerV4 struct {
	v4wire.Packet
	senderKey *ecdsa.PublicKey // used for ping

	// preverify checks whetxer the packet is valid and should be handled at all.
	preverify func(p *packetxandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error
	// handle handles the packet.
	handle func(req *packetxandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte)
}

// PING/v4

func (t *UDPv4) verifyPing(h *packetxandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
...
func (t *UDPv4) handlePing(h *packetxandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
	req := h.Packet.(*v4wire.Ping)
...
func (t *UDPv4) verifyPong(h *packetxandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
...
func (t *UDPv4) verifyFindnode(h *packetxandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
...
func (t *UDPv4) handleFindnode(h *packetxandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
	req := h.Packet.(*v4wire.Findnode)
...
func (t *UDPv4) verifyNeighbors(h *packetxandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
...
func (t *UDPv4) verifyENRRequest(h *packetxandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
...
func (t *UDPv4) handleENRRequest(h *packetxandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
...
func (t *UDPv4) verifyENRResponse(h *packetxandlerV4, from *net.UDPAddr, fromID enode.ID, fromKey v4wire.Pubkey) error {
	if !t.handleReply(fromID, from.IP, h.Packet) {
		return errUnsolicitedReply
	}
	return nil
