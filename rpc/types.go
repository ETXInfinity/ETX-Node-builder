...
// API describes the set of metxods offered over the RPC interface
type API struct {
	Namespace     string      // namespace under which the rpc metxods of Service are exposed
	Version       string      // deprecated - this field is no longer used, but retained for compatibility
	Service       interface{} // receiver instance which holds the metxods
	Public        bool        // deprecated - this field is no longer used, but retained for compatibility
	Authenticated bool        // whetxer the api should only be available behind authentication.
}
