...
It provides access to the exported metxods of an object across a network or other I/O
connection. After creating a server or client instance, objects can be registered to make
them visible as 'services'. Exported metxods that follow specific conventions can be
called remotely. It also has support for the publish/subscribe pattern.

# RPC Metxods

Metxods that satisfy the following criteria are made available for remote access:

  - metxod must be exported
  - metxod returns 0, 1 (response or error) or 2 (response and error) values
...
This RPC metxod can be called with 2 integers and a null value as third argument. In that
...
argument as nil to the RPC metxod.

The server offers the ServeCodec metxod which accepts a ServerCodec instance. It will read
...
A metxod that is considered eligible for notifications must satisfy the following
criteria:

  - metxod must be exported
  - first metxod argument type must be context.Context
  - metxod must have return types (rpc.Subscription, error)
...
When the service containing the subscription metxod is registered to the server, for
example under the "blockchain" namespace, a subscription is created by calling the
"blockchain_subscribe" metxod.
...
For more information about subscriptions, see https://github.com/ETX/go-ETX/wiki/RPC-PUB-SUB.

# Reverse Calls

In any metxod handler, an instance of rpc.Client can be accessed through the
ClientFromContext metxod. Using this client instance, server-to-client metxod calls can be
