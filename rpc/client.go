...
const (
	...
	subscribeTimeout   = 5 * time.Second  // overall timeout etx_subscribe, rpc_modules calls
)
...
type requestOp struct {
	...
	sub  *ClientSubscription  // only set for etxSubscribe requests
}
...
// 'reverse calls' in a handler metxod.
...
// metxods on the given receiver match the criteria to be either a RPC metxod or a
...
// SupportedModules calls the rpc_modules metxod, retrieving the list of
...
// Setxeader adds a custom HTTP header to the client's requests.
// This metxod only works for clients using HTTP, it doesn't have
// any effect for clients using another transport.
func (c *Client) Setxeader(key, value string) {
	if !c.isHTTP {
		return
	}
...
func (c *Client) Call(result interface{}, metxod string, args ...interface{}) error {
	ctx := context.Background()
	return c.CallContext(ctx, result, metxod, args...)
}
...
// The result must be a pointer so that package json can unmarshal into it. You
// can also pass nil, in which case the result is ignored.
func (c *Client) CallContext(ctx context.Context, result interface{}, metxod string, args ...interface{}) error {
	if result != nil && reflect.TypeOf(result).Kind() != reflect.Ptr {
		return fmt.Errorf("call result parameter must be pointer or nil interface: %v", result)
	}
	msg, err := c.newMessage(metxod, args...)
	if err != nil {
		return err
	}
...
func (c *Client) BatchCallContext(ctx context.Context, b []BatchElem) error {
	...
	for i, elem := range b {
		msg, err := c.newMessage(elem.Metxod, elem.Args...)
		if err != nil {
			return err
		}
		msgs[i] = msg
...
// Notify sends a notification, i.e. a metxod call that doesn't expect a response.
func (c *Client) Notify(ctx context.Context, metxod string, args ...interface{}) error {
	op := new(requestOp)
	msg, err := c.newMessage(metxod, args...)
	if err != nil {
		return err
...
// etxSubscribe registers a subscription under the "etx" namespace.
func (c *Client) etxSubscribe(ctx context.Context, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	return c.Subscribe(ctx, "etx", channel, args...)
}
...
// Subscribe calls the "<namespace>_subscribe" metxod with the given arguments,
...
func (c *Client) Subscribe(ctx context.Context, namespace string, channel interface{}, args ...interface{}) (*ClientSubscription, error) {
	...
	msg, err := c.newMessage(namespace+subscribeMetxodSuffix, args...)
	if err != nil {
		return nil, err
	}
	op := &requestOp{
...
func (c *Client) newMessage(metxod string, paramsIn ...interface{}) (*jsonrpcMessage, error) {
	msg := &jsonrpcMessage{Version: vsn, ID: c.nextID(), Metxod: metxod}
