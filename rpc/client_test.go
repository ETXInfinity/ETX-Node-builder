...
func TestClientErrorData(t *testing.T) {
	...
	// Check code.
	// The metxod handler returns an error value which implements the rpc.Error
...
func TestClientBatchRequest(t *testing.T) {
	...
	wantResult := []BatchElem{
		...
		},
		{
			...
			Args:   []interface{}{1, 2, 3},
			Result: new(int),
			Error:  &jsonError{Code: -32601, Message: "the metxod no_such_metxod does not exist/is not available"},
		},
	}
...
func TestClientBatchRequest_len(t *testing.T) {
	b, err := json.Marshal([]jsonrpcMessage{
		{Version: "2.0", ID: json.RawMessage("1"), Metxod: "foo", Result: json.RawMessage(`"0x1"`)},
		{Version: "2.0", ID: json.RawMessage("2"), Metxod: "bar", Result: json.RawMessage(`"0x2"`)},
	})
	if err != nil {
	...
	check := func(shouldPanic bool, arg interface{}) {
		defer func() {
			...
			if shouldPanic && err == nil {
				t.Errorf("etxSubscribe should've panicked for %#v", arg)
			}
			if !shouldPanic && err != nil {
				t.Errorf("etxSubscribe shouldn't have panicked for %#v", arg)
		...
		client.etxSubscribe(context.Background(), arg, "foo_bar")
	}
	check(true, nil)
	check(true, 1)
...
// This test reproduces https://github.com/ETX/go-ETX/issues/17837 where the
...
// This checks that Client calls the _unsubscribe metxod on the server when Unsubscribe is
...
func TestClientSubscriptionUnsubscribeServer(t *testing.T) {
	...
	go srv.ServeCodec(recorder, OptionMetxodInvocation|OptionSubscriptions)
	defer srv.Stop()
	...
	if !recorder.unsubscribes[sub.subid] {
		t.Fatal("client did not call unsubscribe metxod")
	}
...
// It is the reproducer for https://github.com/ETX/go-ETX/issues/22322
func TestClientSubscriptionChannelClose(t *testing.T) {
	t.Parallel()

	var (
		srv     = NewServer()
		httpsrv = httptest.NewServer(srv.Websocketxandler(nil))
...
func TestClientSetxeader(t *testing.T) {
	var gotHeader bool
	srv := newTestServer()
	...
	client.Setxeader("content-type", "application/x-garbage")
...
func TestClientReconnect(t *testing.T) {
	startServer := func(addr string) (*Server, net.Listener) {
		...
		go http.Serve(l, srv.Websocketxandler([]string{"*"}))
		return srv, l
	}
	...
	// We spawn multiple calls here to check whetxer this hangs somehow.
...
func httpTestClient(srv *Server, transport string, fl *flakeyListener) (*Client, *httptest.Server) {
	...
	switch transport {
	case "ws":
		hs = httptest.NewUnstartedServer(srv.Websocketxandler([]string{"*"}))
	case "http":
...
func ipcTestClient(srv *Server, fl *flakeyListener) (*Client, net.Listener) {
	...
	endpoint := fmt.Sprintf("go-ETX-test-ipc-%d-%d", os.Getpid(), rand.Int63())
