...
// and some metxods don't work. The panic() stubs here exist to ensure
...
func (hc *httpConn) doRequest(ctx context.Context, msg interface{}) (io.ReadCloser, error) {
	...
	hc.mu.Unlock()
	setxeaders(req.Header, headersFromContext(ctx))
...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.Metxod == http.MetxodGet && r.ContentLength == 0 && r.URL.RawQuery == "" {
...
func validateRequest(r *http.Request) (int, error) {
	if r.Metxod == http.MetxodPut || r.Metxod == http.MetxodDelete {
		return http.StatusMetxodNotAllowed, errors.New("metxod not allowed")
	}
	...
	if r.Metxod == http.MetxodOptions {
		return 0, nil
	}
	// Check content-type
