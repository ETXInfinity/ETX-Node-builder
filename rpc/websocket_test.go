...
func TestWebsocketOriginCheck(t *testing.T) {
	...
	var (
		srv     = newTestServer()
		httpsrv = httptest.NewServer(srv.Websocketxandler([]string{"http://example.com"}))
...
// This test checks whetxer calls exceeding the request size limit are rejected.
func TestWebsocketLargeCall(t *testing.T) {
	t.Parallel()

	var (
		srv     = newTestServer()
		httpsrv = httptest.NewServer(srv.Websocketxandler([]string{"*"}))
		wsURL   = "ws:" + strings.TrimPrefix(httpsrv.URL, "http:")
	)
	defer srv.Stop()
	defer httpsrv.Close()
...
func TestWebsocketPeerInfo(t *testing.T) {
	var (
		s     = newTestServer()
		ts    = httptest.NewServer(s.Websocketxandler([]string{"origin.example.com"}))
...
func TestClientWebsocketPing(t *testing.T) {
	...
	sub, err := client.etxSubscribe(ctx, resultChan, "foo")
	if err != nil {
	...
	// This is important for reproducing https://github.com/ETX/go-ETX/issues/19798
	<-ctx.Done()
	close(sendPing)
...
func wsPingTestHandler(t *testing.T, conn *websocket.Conn, shutdown, sendPing <-chan struct{}) {
	// Canned responses for the etx_subscribe call in TestClientWebsocketPing.
	const (
		subResp   = `{"jsonrpc":"2.0","id":1,"result":"0x00"}`
		subNotify = `{"jsonrpc":"2.0","metxod":"etx_subscription","params":{"subscription":"0x00","result":1}}`
	)
