...
func makeChain(n int, seed byte, parent *types.Block) ([]common.Hash, map[common.Hash]*types.Block) {
	blocks, _ := core.GenerateChain(gspec.Config, parent, etxash.NewFaker(), testdb, n, func(i int, block *core.BlockGen) {
...
func newTester(light bool) *fetcherTester {
	...
	tester.fetcher = NewBlockFetcher(light, tester.getxeader, tester.getBlock, tester.verifyHeader, tester.broadcastBlock, tester.chainHeight, tester.insertHeaders, tester.insertChain, tester.dropPeer)
	tester.fetcher.Start()

	return tester
}

// getxeader retrieves a header from the tester's block chain.
func (f *fetcherTester) getxeader(hash common.Hash) *types.Header {
	f.lock.RLock()
...
func (f *fetcherTester) makeHeaderFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) headerRequesterFn {
	...
	return func(hash common.Hash, sink chan *etx.Response) (*etx.Request, error) {
...
func (f *fetcherTester) makeBodyFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	...
	return func(hashes []common.Hash, sink chan *etx.Response) (*etx.Request, error) {
		...
		// Return on a new thread
		bodies := make([]*etx.BlockBody, len(transactions))
		for i, txs := range transactions {
			bodies[i] = &etx.BlockBody{
				Transactions: txs,
				Uncles:       uncles[i],
			}
		}
...
func testConcurrentAnnouncements(t *testing.T, light bool) {
	...
	counter := uint32(0)
	firstHeaderWrapper := func(hash common.Hash, sink chan *etx.Response) (*etx.Request, error) {
		atomic.AddUint32(&counter, 1)
		return firstHeaderFetcher(hash, sink)
	}
	secondHeaderWrapper := func(hash common.Hash, sink chan *etx.Response) (*etx.Request, error) {
		atomic.AddUint32(&counter, 1)
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	counter := uint32(0)
	headerWrapper := func(hash common.Hash, sink chan *etx.Response) (*etx.Request, error) {
		atomic.AddUint32(&counter, 1)

		// Simulate a long running fetch
		resink := make(chan *etx.Response)
	...
	if light {
		checkNonExist = func() bool {
			return tester.getxeader(hashes[0]) == nil
		}
	}
