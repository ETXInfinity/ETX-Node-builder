...
// ETXAPI provides an API to access ETX full node-related information.
type ETXAPI struct {
	e *ETX
}

// NewETXAPI creates a new ETX protocol API for full nodes.
func NewETXAPI(e *ETX) *ETXAPI {
	return &ETXAPI{e}
}

// etxerbase is the address that mining rewards will be send to.
func (api *ETXAPI) etxerbase() (common.Address, error) {
	return api.e.etxerbase()
}

// Coinbase is the address that mining rewards will be send to (alias for etxerbase).
func (api *ETXAPI) Coinbase() (common.Address, error) {
	return api.etxerbase()
}

// Hashrate returns the POW hashrate.
func (api *ETXAPI) Hashrate() hexutil.Uint64 {
	return hexutil.Uint64(api.e.Miner().Hashrate())
}

// Mining returns an indication if this node is currently mining.
func (api *ETXAPI) Mining() bool {
...
func NewMinerAPI(e *ETX) *MinerAPI {
	return &MinerAPI{e}
}

// Start starts the miner with the given number of threads. If threads is nil,
// the number of workers started is equal to the number of logical CPUs that are
// usable by this process. If mining is already running, this metxod adjust the
...
// Setetxerbase sets the etxerbase of the miner.
func (api *MinerAPI) Setetxerbase(etxerbase common.Address) bool {
	api.e.Setetxerbase(etxerbase)
	return true
}
...
// AdminAPI is the collection of ETX full node related APIs for node
// administration.
...
func NewAdminAPI(etx *ETX) *AdminAPI {
	return &AdminAPI{etx: etx}
}
...
func (api *AdminAPI) ExportChain(file string, first *uint64, last *uint64) (bool, error) {
	...
	if first != nil && last == nil {
		head := api.etx.BlockChain().CurrentHeader().Number.Uint64()
		last = &head
	}
	...
	if first != nil {
		if err := api.etx.BlockChain().ExportN(writer, *first, *last); err != nil {
			return false, err
		}
	} else if err := api.etx.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *AdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.etx.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.etx.BlockChain().InsertChain(blocks); err != nil {
...
// DebugAPI is the collection of ETX full node APIs for debugging the
// protocol.
...
func NewDebugAPI(etx *ETX) *DebugAPI {
	return &DebugAPI{etx: etx}
}
...
func (api *DebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	...
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.etx.miner.Pending()
		return stateDb.RawDump(opts), nil
	}
	var block *types.Block
	if blockNr == rpc.LatestBlockNumber {
		block = api.etx.blockchain.CurrentBlock()
	} else if blockNr == rpc.FinalizedBlockNumber {
		block = api.etx.blockchain.CurrentFinalizedBlock()
	} else if blockNr == rpc.SafeBlockNumber {
		block = api.etx.blockchain.CurrentSafeBlock()
	} else {
		block = api.etx.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
		return state.Dump{}, fmt.Errorf("block #%d not found", blockNr)
	}
	stateDb, err := api.etx.BlockChain().StateAt(block.Root())
	if err != nil {
...
func (api *DebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	if preimage := rawdb.ReadPreimage(api.etx.ChainDb(), hash); preimage != nil {
		return preimage, nil
	}
...
func (api *DebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	var (
		err     error
		blocks  = rawdb.ReadAllBadBlocks(api.etx.chainDb)
	...
	for _, block := range blocks {
		...
		if blockJSON, err = etxapi.RPCMarshalBlock(block, true, true, api.etx.APIBackend.ChainConfig()); err != nil {
	...
	if number, ok := blockNrOrHash.Number(); ok {
		if number == rpc.PendingBlockNumber {
			...
			if number == rpc.LatestBlockNumber {
				...
				block = api.etx.blockchain.GetBlockByNumber(uint64(number))
			}
			if block == nil {
		...
		block := api.etx.blockchain.GetBlockByHash(hash)
		if block == nil {
...
func (api *DebugAPI) StorageRangeAt(blockHash common.Hash, txIndex int, contractAddress common.Address, keyStart hexutil.Bytes, maxResult int) (StorageRangeResult, error) {
	// Retrieve the block
	block := api.etx.blockchain.GetBlockByHash(blockHash)
	if block == nil {
		return StorageRangeResult{}, fmt.Errorf("block %#x not found", blockHash)
	}
	_, _, statedb, release, err := api.etx.stateAtTransaction(block, txIndex, 0)
	if err != nil {
...
func (api *DebugAPI) GetModifiedAccountsByNumber(startNum uint64, endNum *uint64) ([]common.Address, error) {
	...
	startBlock = api.etx.blockchain.GetBlockByNumber(startNum)
	if startBlock == nil {
	...
	if endNum == nil {
		endBlock = startBlock
		startBlock = api.etx.blockchain.GetBlockByHash(startBlock.ParentHash())
		if startBlock == nil {
			return nil, fmt.Errorf("block %x has no parent", endBlock.Number())
		}
	} else {
		endBlock = api.etx.blockchain.GetBlockByNumber(*endNum)
		if endBlock == nil {
...
func (api *DebugAPI) GetModifiedAccountsByHash(startHash common.Hash, endHash *common.Hash) ([]common.Address, error) {
	var startBlock, endBlock *types.Block
	startBlock = api.etx.blockchain.GetBlockByHash(startHash)
	if startBlock == nil {
	...
	if endHash == nil {
		...
		endBlock = api.etx.blockchain.GetBlockByHash(*endHash)
		if endBlock == nil {
...
func (api *DebugAPI) getModifiedAccounts(startBlock, endBlock *types.Block) ([]common.Address, error) {
	...
	triedb := api.etx.BlockChain().StateCache().TrieDB()
...
func (api *DebugAPI) GetAccessibleState(from, to rpc.BlockNumber) (uint64, error) {
	...
	for i := int64(start); i != int64(end); i += delta {
		...
		h := api.etx.BlockChain().GetxeaderByNumber(uint64(i))
		if h == nil {
			return 0, fmt.Errorf("missing header %d", i)
		}
		if ok, _ := api.etx.ChainDb().Has(h.Root[:]); ok {
			return uint64(i), nil
		}
	}
