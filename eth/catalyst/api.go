...
// Package catalyst implements the temporary etx1/etx2 RPC integration.
package catalyst
...
func Register(stack *node.Node, backend *etx.ETX) error {
	log.Warn("Engine API enabled", "protocol", "etx")
...
type ConsensusAPI struct {
	...
	// The forkchoice update and new payload metxod require us to return the
	...
	//     persist any bad block information to disk as a bug in Getx could end
	//     up blocking a valid chain, even if a later Getx update would accept
	//     it.
	//   - Bad blocks will get forgotten after a certain threshold of import
	//     attempts and will be retried. The rationale is that if the network
	//     really-really-really tries to feed us a block, we should give it a
	//     new chance, perhaps us being racey instead of the block being legit
	//     bad (this happened in Getx at a point with import vs. pending race).
	...
	// Getx can appear to be stuck or do strange things if the beacon client is
	...
	forkchoiceLock sync.Mutex // Lock for the forkChoiceUpdated metxod
	newPayloadLock sync.Mutex // Lock for the NewPayload metxod
}

// NewConsensusAPI creates a new consensus api for the given backend.
// The underlying blockchain needs to have a valid terminal total difficulty set.
func NewConsensusAPI(etx *etx.ETX) *ConsensusAPI {
	if etx.BlockChain().Config().TerminalTotalDifficulty == nil {
	...
	etx.Downloader().SetBadBlockCallback(api.setInvalidAncestor)
	go api.heartbeat()
...
//
// If the metxod is called with an empty head block: we return success, which can be used
...
func (api *ConsensusAPI) ForkchoiceUpdatedV1(update beacon.ForkchoiceStateV1, payloadAttributes *beacon.PayloadAttributesV1) (beacon.ForkChoiceResponse, error) {
	...
	log.Trace("Engine API request received", "metxod", "ForkchoiceUpdated", "head", update.HeadBlockHash, "finalized", update.FinalizedBlockHash, "safe", update.SafeBlockHash)
	...
	// Check whetxer we have the block yet in our database or not. If not, we'll
	// need to either trigger a sync, or to reject this forkchoice update for a
	// reason.
	block := api.etx.BlockChain().GetBlockByHash(update.HeadBlockHash)
	if block == nil {
		...
		// the future to resolve from the `etx` network, but it's an unexpected case
		...
		if merger := api.etx.Merger(); !merger.TDDReached() {
			merger.ReachTTD()
			api.etx.Downloader().Cancel()
		}
		log.Info("Forkchoice requested sync to new head", "number", header.Number, "hash", header.Hash())
		if err := api.etx.Downloader().BeaconSync(api.etx.SyncMode(), header); err != nil {
	...
	if block.Difficulty().BitLen() > 0 || block.NumberU64() == 0 {
		var (
			td  = api.etx.BlockChain().GetTd(update.HeadBlockHash, block.NumberU64())
			ptd = api.etx.BlockChain().GetTd(block.ParentHash(), block.NumberU64()-1)
			ttd = api.etx.BlockChain().Config().TerminalTotalDifficulty
		)
	...
	if rawdb.ReadCanonicalHash(api.etx.ChainDb(), block.NumberU64()) != update.HeadBlockHash {
		// Block is not canonical, set head.
		if latestValid, err := api.etx.BlockChain().SetCanonical(block); err != nil {
	...
	} else if api.etx.BlockChain().CurrentBlock().Hash() == update.HeadBlockHash {
		...
		log.Info("Ignoring beacon update to old head", "number", block.NumberU64(), "hash", update.HeadBlockHash, "age", common.PrettyAge(time.Unix(int64(block.Time()), 0)), "have", api.etx.BlockChain().CurrentBlock().NumberU64())
	...
	if update.FinalizedBlockHash != (common.Hash{}) {
		if merger := api.etx.Merger(); !merger.PoSFinalized() {
			merger.FinalizePoS()
		}
		// If the finalized block is not in our canonical tree, sometxings wrong
		finalBlock := api.etx.BlockChain().GetBlockByHash(update.FinalizedBlockHash)
		if finalBlock == nil {
		...
		} else if rawdb.ReadCanonicalHash(api.etx.ChainDb(), finalBlock.NumberU64()) != update.FinalizedBlockHash {
		...
		api.etx.BlockChain().SetFinalized(finalBlock)
	}
	// Check if the safe block hash is in our canonical tree, if not sometxings wrong
	if update.SafeBlockHash != (common.Hash{}) {
		safeBlock := api.etx.BlockChain().GetBlockByHash(update.SafeBlockHash)
		if safeBlock == nil {
			log.Warn("Safe block not available in database")
			return beacon.STATUS_INVALID, beacon.InvalidForkChoiceState.With(errors.New("safe block not available in database"))
		}
		if rawdb.ReadCanonicalHash(api.etx.ChainDb(), safeBlock.NumberU64()) != update.SafeBlockHash {
			log.Warn("Safe block not in canonical chain")
			return beacon.STATUS_INVALID, beacon.InvalidForkChoiceState.With(errors.New("safe block not in canonical chain"))
		}
		// Set the safe block
		api.etx.BlockChain().SetSafe(safeBlock)
	}
	...
	if payloadAttributes != nil {
		...
		payload, err := api.etx.Miner().BuildPayload(args)
		if err != nil {
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config beacon.TransitionConfigurationV1) (*beacon.TransitionConfigurationV1, error) {
	log.Trace("Engine API request received", "metxod", "ExchangeTransitionConfiguration", "ttd", config.TerminalTotalDifficulty)
	...
	if ttd == nil || ttd.Cmp(config.TerminalTotalDifficulty.ToInt()) != 0 {
		log.Warn("Invalid TTD configured", "getx", ttd, "beacon", config.TerminalTotalDifficulty)
		return nil, fmt.Errorf("invalid ttd: execution %v consensus %v", ttd, config.TerminalTotalDifficulty)
	}
	if config.TerminalBlockHash != (common.Hash{}) {
		if hash := api.etx.BlockChain().GetCanonicalHash(uint64(config.TerminalBlockNumber)); hash == config.TerminalBlockHash {
...
func (api *ConsensusAPI) GetPayloadV1(payloadID beacon.PayloadID) (*beacon.ExecutableDataV1, error) {
	log.Trace("Engine API request received", "metxod", "GetPayload", "id", payloadID)
...
// NewPayloadV1 creates an etx1 block, inserts it in the chain, and returns the status of the chain.
func (api *ConsensusAPI) NewPayloadV1(params beacon.ExecutableDataV1) (beacon.PayloadStatusV1, error) {
	...
	//      api.etx.BlockChain().InsertBlockWithoutSetxead, where it is blocked on
	...
	// check whetxer we already have the block locally.
	api.newPayloadLock.Lock()
	defer api.newPayloadLock.Unlock()

	log.Trace("Engine API request received", "metxod", "ExecutePayload", "number", params.Number, "hash", params.BlockHash)
	...
	if block := api.etx.BlockChain().GetBlockByHash(params.BlockHash); block != nil {
	...
	parent := api.etx.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
	...
	var (
		ptd  = api.etx.BlockChain().GetTd(parent.Hash(), parent.NumberU64())
		ttd  = api.etx.BlockChain().Config().TerminalTotalDifficulty
		gptd = api.etx.BlockChain().GetTd(parent.ParentHash(), parent.NumberU64()-1)
	)
	if ptd.Cmp(ttd) < 0 {
	...
	// tries to make it import a block. That should be denied as pushing sometxing
	// into the database directly will conflict with the assumptions of snap sync
	// that it has an empty db that it can fill itself.
	if api.etx.SyncMode() != downloader.FullSync {
		return api.delayPayloadImport(block)
	}
	if !api.etx.BlockChain().HasBlockAndState(block.ParentHash(), block.NumberU64()-1) {
		api.remoteBlocks.put(block.Hash(), block.Header())
		log.Warn("State not available, ignoring new payload")
		return beacon.PayloadStatusV1{Status: beacon.ACCEPTED}, nil
	}
	log.Trace("Inserting block without setxead", "hash", block.Hash(), "number", block.Number)
	if err := api.etx.BlockChain().InsertBlockWithoutSetxead(block); err != nil {
...
// either via a forkchoice update or a sync extension. This metxod is meant to
...
func (api *ConsensusAPI) delayPayloadImport(block *types.Block) (beacon.PayloadStatusV1, error) {
	...
	if err := api.etx.Downloader().BeaconExtend(api.etx.SyncMode(), block.Header()); err == nil {
...
// checkInvalidAncestor checks whetxer the specified chain end links to a known
...
func (api *ConsensusAPI) checkInvalidAncestor(check common.Hash, head common.Hash) *beacon.PayloadStatusV1 {
	...
	if header := api.etx.BlockChain().Getxeader(invalid.ParentHash, invalid.Number.Uint64()-1); header != nil && header.Difficulty.Sign() != 0 {
...
func (api *ConsensusAPI) invalid(err error, latestValid *types.Header) beacon.PayloadStatusV1 {
	currentHash := api.etx.BlockChain().CurrentBlock().Hash()
	if latestValid != nil {
...
// user that sometxing might be off with their consensus node.
//
...
func (api *ConsensusAPI) heartbeat() {
	...
	for {
		...
		if api.etx.BlockChain().Config().TerminalTotalDifficultyPassed || api.etx.Merger().TDDReached() {
		...
		if time.Since(offlineLogged) > beaconUpdateWarnFrequency {
			...
			if head.Number.Uint64() > 0 {
				...
				for i := 0; i < 64; i++ {
					parent := chain.Getxeader(current.ParentHash, current.Number.Uint64()-1)
					if parent == nil {
						break
					}
