...
// FullSyncTester is an auxiliary service that allows Getx to perform full sync
...
func RegisterFullSyncTester(stack *node.Node, backend *etx.ETX, block *types.Block) (*FullSyncTester, error) {
	cl := &FullSyncTester{
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				if tester.api.etx.Downloader().Synchronising() {
					continue
				}
				// Short circuit in case the target block is already stored
				// locally.
				if tester.api.etx.BlockChain().HasBlock(tester.block.Hash(), tester.block.NumberU64()) {
