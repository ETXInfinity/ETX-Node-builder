...
func generatePreMergeChain(n int) (*core.Genesis, []*types.Block) {
	...
	_, blocks, _ := core.GenerateChainWithGenesis(genesis, etxash.NewFaker(), n, generate)
...
func Testetx2AssembleBlock(t *testing.T) {
	genesis, blocks := generatePreMergeChain(10)
	n, etxservice := startetxService(t, genesis, blocks)
	defer n.Close()

	api := NewConsensusAPI(etxservice)
	signer := types.NewEIP155Signer(etxservice.BlockChain().Config().ChainID)
...
func Testetx2AssembleBlockWithAnotherBlocksTxs(t *testing.T) {
	genesis, blocks := generatePreMergeChain(10)
	n, etxservice := startetxService(t, genesis, blocks[:9])
	defer n.Close()

	api := NewConsensusAPI(etxservice)

	// Put the 10th block's tx in the pool and produce a new block
	api.etx.TxPool().AddRemotesSync(blocks[9].Transactions())
	blockParams := beacon.PayloadAttributesV1{
...
func TestSetxeadBeforeTotalDifficulty(t *testing.T) {
	genesis, blocks := generatePreMergeChain(10)
	n, etxservice := startetxService(t, genesis, blocks)
	defer n.Close()
...
func Testetx2PrepareAndGetPayload(t *testing.T) {
	genesis, blocks := generatePreMergeChain(10)
	...
	// Put the 10th block's tx in the pool and produce a new block
	etxservice.TxPool().AddLocals(blocks[9].Transactions())
	blockParams := beacon.PayloadAttributesV1{
		Timestamp: blocks[8].Time() + 5,
	}
	fcState := beacon.ForkchoiceStateV1{
...
func TestInvalidPayloadTimestamp(t *testing.T) {
	...
	n, etxservice := startetxService(t, genesis, preMergeBlocks)
	defer n.Close()
	var (
		api    = NewConsensusAPI(etxservice)
		parent = etxservice.BlockChain().CurrentBlock()
	)
	tests := []struct {
		time      uint64
		shouldErr bool
...
func Testetx2NewBlock(t *testing.T) {
	genesis, preMergeBlocks := generatePreMergeChain(10)
	n, etxservice := startetxService(t, genesis, preMergeBlocks)
	defer n.Close()
	...
	etxservice.BlockChain().SubscribeLogsEvent(newLogCh)
	etxservice.BlockChain().SubscribeRemovedLogsEvent(rmLogsCh)

	for i := 0; i < 10; i++ {
		statedb, _ := etxservice.BlockChain().StateAt(parent.Root())
		nonce := statedb.GetNonce(testAddr)
		tx, _ := types.SignTx(types.NewContractCreation(nonce, new(big.Int), 1000000, big.NewInt(2*params.InitialBaseFee), logCode), types.LatestSigner(etxservice.BlockChain().Config()), testKey)
		etxservice.TxPool().AddLocal(tx)
		...
		case etxservice.BlockChain().CurrentBlock().NumberU64() != block.NumberU64()-1:
		...
		if have, want := etxservice.BlockChain().CurrentBlock().NumberU64(), block.NumberU64(); have != want {
	...
	var (
		head = etxservice.BlockChain().CurrentBlock().NumberU64()
	)
	parent = preMergeBlocks[len(preMergeBlocks)-1]
	for i := 0; i < 10; i++ {
		execData, err := assembleBlock(api, parent.Hash(), &beacon.PayloadAttributesV1{
		...
		if etxservice.BlockChain().CurrentBlock().NumberU64() != head {
			t.Fatalf("Chain head shouldn't be updated")
		}
		...
		if etxservice.BlockChain().CurrentBlock().NumberU64() != block.NumberU64() {
...
func Testetx2DeepReorg(t *testing.T) {
	// TODO (MariusVanDerWijden) Testetx2DeepReorg is currently broken, because it tries to reorg
	...
	/*
		...
		var (
			api    = NewConsensusAPI(etxservice, nil)
			parent = preMergeBlocks[len(preMergeBlocks)-core.TriesInMemory-1]
			head   = etxservice.BlockChain().CurrentBlock().NumberU64()
		)
		if etxservice.BlockChain().HasBlockAndState(parent.Hash(), parent.NumberU64()) {
		...
		for i := 0; i < 10; i++ {
			...
			block, err := ExecutableDataToBlock(etxservice.BlockChain().Config(), parent.Header(), *execData)
			if err != nil {
				t.Fatalf("Failed to convert executable data to block %v", err)
			}
			...
			if err := api.setxead(block.Hash()); err != nil {
...
// startetxService creates a full node instance for testing.
func startetxService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *etx.ETX) {
	t.Helper()
	...
	etxcfg := &etxconfig.Config{Genesis: genesis, etxash: etxash.Config{PowMode: etxash.ModeFake}, SyncMode: downloader.FullSync, TrieTimeout: time.Minute, TrieDirtyCache: 256, TrieCleanCache: 256}
	etxservice, err := etx.New(n, etxcfg)
	if err != nil {
		t.Fatal("can't create etx service:", err)
	}
	if err := n.Start(); err != nil {
		t.Fatal("can't start node:", err)
	}
	if _, err := etxservice.BlockChain().InsertChain(blocks); err != nil {
		n.Close()
		t.Fatal("can't import test blocks:", err)
	}

	etxservice.Setetxerbase(testAddr)
	etxservice.SetSynced()
	return n, etxservice
}
...
func TestFullAPI(t *testing.T) {
	...
	setupBlocks(t, etxservice, 10, parent, callback)
}

func setupBlocks(t *testing.T, etxservice *etx.ETX, n int, parent *types.Block, callback func(parent *types.Block)) {
	api := NewConsensusAPI(etxservice)
	for i := 0; i < n; i++ {
		callback(parent)
		...
		if etxservice.BlockChain().CurrentBlock().NumberU64() != payload.Number {
			t.Fatal("Chain head should be updated")
		}
		if etxservice.BlockChain().CurrentFinalizedBlock().NumberU64() != payload.Number-1 {
...
func TestNewPayloadOnInvalidChain(t *testing.T) {
	...
	var (
		api    = NewConsensusAPI(etxservice)
		parent = etxservice.BlockChain().CurrentBlock()
		signer = types.LatestSigner(etxservice.BlockChain().Config())
		// This EVM code generates a log when the contract is created.
		logCode = common.Hex2Bytes("60606040525b7f24ec1d3ff24c2f6ff210738839dbc339cd45a5294d85c79361016243157aae7b60405180905060405180910390a15b600a8060416000396000f360606040526008565b00")
	)
	for i := 0; i < 10; i++ {
		...
		etxservice.TxPool().AddRemotesSync([]*types.Transaction{tx})
		var (
			params = beacon.PayloadAttributesV1{
...
func TestEmptyBlocks(t *testing.T) {
	...
	commonAncestor := etxservice.BlockChain().CurrentBlock()
	api := NewConsensusAPI(etxservice)

	// Setup 10 blocks on the canonical chain
	setupBlocks(t, etxservice, 10, commonAncestor, func(parent *types.Block) {})
...
func TestTrickRemoteBlockCache(t *testing.T) {
	// Setup two nodes
	genesis, preMergeBlocks := generatePreMergeChain(10)
	nodeA, etxserviceA := startetxService(t, genesis, preMergeBlocks)
	nodeB, etxserviceB := startetxService(t, genesis, preMergeBlocks)
	defer nodeA.Close()
	defer nodeB.Close()
	...
	apiA := NewConsensusAPI(etxserviceA)
	apiB := NewConsensusAPI(etxserviceB)

	commonAncestor := etxserviceA.BlockChain().CurrentBlock()

	// Setup 10 blocks on the canonical chain
	setupBlocks(t, etxserviceA, 10, commonAncestor, func(parent *types.Block) {})
	commonAncestor = etxserviceA.BlockChain().CurrentBlock()
...
func TestSimultaneousNewBlock(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		statedb, _ := etxservice.BlockChain().StateAt(parent.Root())
		etxservice.TxPool().AddLocal(types.MustSignNewTx(testKey, types.LatestSigner(etxservice.BlockChain().Config()),
			&types.DynamicFeeTx{
				Nonce:     statedb.GetNonce(testAddr),
				Value:     big.NewInt(0),
		...
		if etxservice.BlockChain().CurrentBlock().NumberU64() != block.NumberU64()-1 {
			t.Fatalf("Chain head shouldn't be updated")
		}
		fcState := beacon.ForkchoiceStateV1{
			HeadBlockHash:      block.Hash(),
