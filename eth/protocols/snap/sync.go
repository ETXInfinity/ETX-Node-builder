...
import (
	...
	"github.com/ETX/go-ETX/p2p/msgrate"
...
type accountResponse struct {
	...
	cont bool // Whetxer the account range has a continuation
}
...
type storageResponse struct {
	...
	cont bool // Whetxer the last storage range has a continuation
}
...
type accountTask struct {
	...
	needCode  []bool // Flags whetxer the filling accounts need code retrieval
	needState []bool // Flags whetxer the filling accounts need storage retrieval
	needHeal  []bool // Flags whetxer the filling accounts's state was chunked and need healing

	codeTasks  map[common.Hash]struct{}    // Code hashes that need retrieval
	stateTasks map[common.Hash]common.Hash // Account hashes->roots that need full state retrieval

	genBatch etxdb.Batch     // Batch used by the node generator
	genTrie  *trie.StackTrie // Node generator from storage slots

	done bool // Flag whetxer the task can be removed
}
...
// SyncPeer abstracts out the metxods required for a peer to be synced against
...
// Syncer is an ETX account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db etxdb.KeyValueStore // Database to store the trie nodes into (and dedup)
	...
	stateWriter        etxdb.Batch        // Shared batch writer used for persisting raw states
...
// NewSyncer creates a new snapshot syncer to download the ETX state over the
// snap protocol.
func NewSyncer(db etxdb.KeyValueStore) *Syncer {
	return &Syncer{
		db: db,
...
func (s *Syncer) Sync(root common.Hash, cancel chan struct{}) error {
	...
	// Whetxer sync completed or not, disregard any future packets
	defer func() {
	...
	// ephemeral so a data race doesn't accidentally deliver sometxing stale on
	...
	for {
		...
		s.lock.Unlock()
		// Wait for sometxing to happen
		select {
		case <-s.update:
			// Sometxing happened (new peer, delivery, timeout), recheck tasks
		case <-peerJoin:
		...
		// Report stats if sometxing meaningful happened
		s.report(false)
	}
}
...
func (s *Syncer) loadSyncStatus() {
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				task.genBatch = etxdb.HookedBatch{
					Batch: s.db.NewBatch(),
				...
				for accountHash, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						subtask.genBatch = etxdb.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
...
func (s *Syncer) assignTrienodeHealTasks(success chan *trienodeHealResponse, fail chan *trienodeHealRequest, cancel chan struct{}) {
	...
	for len(s.healer.trieTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		...
		// togetxer with bytecodes, so we need to queue them combined.
		var (
...
func (s *Syncer) assignBytecodeHealTasks(success chan *bytecodeHealResponse, fail chan *bytecodeHealRequest, cancel chan struct{}) {
	...
	for len(s.healer.codeTasks) > 0 || s.healer.scheduler.Pending() > 0 {
		// If there are not enough trie tasks queued to fully assign, fill the
		// queue from the state sync scheduler. The trie synced schedules these
		// togetxer with trie nodes, so we need to queue them combined.
		var (
			have = len(s.healer.trieTasks) + len(s.healer.codeTasks)
			want = maxTrieRequestCount + maxCodeRequestCount
		)
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
	...
	if res.subTask != nil {
		...
		if res.subTask.genBatch.ValueSize() > etxdb.IdealBatchSize || res.subTask.done {
...
func (s *Syncer) commitHealer(force bool) {
	if !force && s.healer.scheduler.MemSize() < etxdb.IdealBatchSize {
		return
	}
	batch := s.db.NewBatch()
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	if task.genBatch.ValueSize() > etxdb.IdealBatchSize || task.done {
		if err := task.genBatch.Write(); err != nil {
...
// OnAccounts is a callback metxod to invoke when a range of accounts are
...
func (s *Syncer) OnAccounts(peer SyncPeer, id uint64, hashes []common.Hash, accounts [][]byte, proof [][]byte) error {
	...
	// Whetxer or not the response is valid, we can mark the peer as idle and
...
// OnByteCodes is a callback metxod to invoke when a batch of contract
...
// OnStorage is a callback metxod to invoke when ranges of storage slots
...
// OnTrieNodes is a callback metxod to invoke when a batch of trie nodes
// are received from a remote peer.
...
// onHealByteCodes is a callback metxod to invoke when a batch of contract
...
// onHealState is a callback metxod to invoke when a flat state(account
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	...
	if s.stateWriter.ValueSize() > etxdb.IdealBatchSize {
...
// a contract storage, based on the number of keys and the last hash. This metxod
