...
// handleGetBlockHeaders66 is the etx/66 version of handleGetBlockHeaders
...
func serviceNonContiguousBlockHeaderQuery(chain *core.BlockChain, query *GetBlockHeadersPacket, peer *Peer) []rlp.RawValue {
	...
	for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit &&
		...
		if hashMode {
			if first {
				first = false
				origin = chain.GetxeaderByHash(query.Origin.Hash)
				if origin != nil {
					query.Origin.Number = origin.Number.Uint64()
				}
			} else {
				origin = chain.Getxeader(query.Origin.Hash, query.Origin.Number)
			}
		} else {
			origin = chain.GetxeaderByNumber(query.Origin.Number)
		}
		if origin == nil {
			break
		}
		...
		case hashMode && !query.Reverse:
			...
			if next <= current {
				...
				if header := chain.GetxeaderByNumber(next); header != nil {
...
func serviceContiguousBlockHeaderQuery(chain *core.BlockChain, query *GetBlockHeadersPacket) []rlp.RawValue {
	...
	if query.Origin.Hash == (common.Hash{}) {
		...
		headers := chain.GetxeadersFrom(from, count)
		if !query.Reverse {
	...
	var (
		...
		header  = chain.GetxeaderByHash(hash)
	)
	if header != nil {
	...
	if !query.Reverse {
		...
		descendants := chain.GetxeadersFrom(num+count-1, count-1)
	...
	{ // Last mode: deliver ancestors of H
		for i := uint64(1); header != nil && i < count; i++ {
			header = chain.GetxeaderByHash(header.ParentHash)
			if header == nil {
				break
			}
			rlpData, _ := rlp.EncodeToBytes(header)
...
func ServiceGetReceiptsQuery(chain *core.BlockChain, query GetReceiptsPacket) []rlp.RawValue {
	...
	for lookups, hash := range query {
		...
		if results == nil {
			if header := chain.GetxeaderByHash(hash); header == nil || header.ReceiptHash != types.EmptyRootHash {
				continue
			}
		}
