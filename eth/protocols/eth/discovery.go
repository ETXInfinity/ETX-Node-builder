...
// enrEntry is the ENR entry which advertises `etx` protocol on the discovery.
...
// StartENRUpdater starts the `etx` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `etx` ENR entry based on the current state of the chain.
