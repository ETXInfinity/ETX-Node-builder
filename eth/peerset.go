...
var (
	...
	// errSnapWithoutetx is returned if a peer attempts to connect only on the
	// snap protocol without advertising the etx main protocol.
	errSnapWithoutetx = errors.New("peer connected on snap without compatible etx support")
)

// peerSet represents the collection of active peers currently participating in
// the `etx` protocol, with or without the `snap` extension.
type peerSet struct {
	peers     map[string]*etxPeer // Peers connected on the `etx` protocol
	snapPeers int                 // Number of `snap` compatible peers for connection prioritization

	snapWait map[string]chan *snap.Peer // Peers connected on `etx` waiting for their snap extension
	snapPend map[string]*snap.Peer      // Peers connected on the `snap` protocol, but not yet on `etx`
...
func newPeerSet() *peerSet {
	return &peerSet{
		peers:    make(map[string]*etxPeer),
		snapWait: make(map[string]chan *snap.Peer),
		snapPend: make(map[string]*snap.Peer),
	}
}

// registerSnapExtension unblocks an already connected `etx` peer waiting for its
// `snap` extension, or if no such peer exists, tracks the extension for the time
// being until the `etx` main protocol starts looking for it.
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	// Reject the peer if it advertises `snap` without `etx` as `snap` is only a
	// satellite protocol meaningful with the chain selection of `etx`
	if !peer.RunningCap(etx.ProtocolName, etx.ProtocolVersions) {
		return errSnapWithoutetx
	}
	...
	// Inject the peer into an `etx` counterpart is available, otherwise save for later
...
// by the peerset.
func (ps *peerSet) waitSnapExtension(peer *etx.Peer) (*snap.Peer, error) {
...
// registerPeer injects a new `etx` peer into the working set, or returns an error
// if the peer is already known.
func (ps *peerSet) registerPeer(peer *etx.Peer, ext *snap.Peer) error {
...
func (ps *peerSet) peer(id string) *etxPeer {
	ps.lock.RLock()
...
func (ps *peerSet) peersWithoutBlock(hash common.Hash) []*etxPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

	list := make([]*etxPeer, 0, len(ps.peers))
...
func (ps *peerSet) peersWithoutTransaction(hash common.Hash) []*etxPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
...
// len returns if the current number of `etx` peers in the set. Since the `snap`
// peers are tied to the existence of an `etx` connection, that will always be a
// subset of `etx`.
...
func (ps *peerSet) peerWithHighestTD() *etx.Peer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
