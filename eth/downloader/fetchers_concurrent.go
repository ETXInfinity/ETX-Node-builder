...
type typedQueue interface {
	...
	request(peer *peerConnection, req *fetchRequest, resCh chan *etx.Response) (*etx.Request, error)
	...
	deliver(peer *peerConnection, packet *etx.Response) (int, error)
}
...
func (d *Downloader) concurrentFetch(queue typedQueue, beaconMode bool) error {
	...
	responses := make(chan *etx.Response)

	// Track the currently active requests and their timeout order
	pending := make(map[string]*etx.Request)
	defer func() {
	...
	ordering := make(map[*etx.Request]int)
	timeouts := prque.New(func(data interface{}, index int) {
		ordering[data.(*etx.Request)] = index
	})
	...
	// the pending request altogetxer as that would prevent a late response from
	// being delivered, thus never unblocking the peer.
	stales := make(map[string]*etx.Request)
	defer func() {
		// Abort all requests on sync cycle cancellation. The requests may still
		// be fulfilled by the remote side, but the dispatcher will not wait to
		// deliver them since nobody's going to be listening.
	...
	for {
		...
		if queue.pending() == 0 {
			...
			for _, peer := range idles {
				...
				if err != nil {
					...
					queue.unreserve(peer.id) // TODO(karalabe): This needs a non-expiration metxod
					continue
				}
				pending[peer.id] = req
		...
		case event := <-peering:
			...
			if req, ok := pending[peerid]; ok {
				queue.unreserve(peerid) // TODO(karalabe): This needs a non-expiration metxod
