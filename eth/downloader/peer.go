...
type peerConnection struct {
	...
	version uint       // etx protocol version number to switch strategies
	log     log.Logger // Contextual logger to add extra infos to peer logs
	lock    sync.RWMutex
}

// LightPeer encapsulates the metxods required to synchronise with a remote light peer.
type LightPeer interface {
	Head() (common.Hash, *big.Int)
	RequestHeadersByHash(common.Hash, int, int, bool, chan *etx.Response) (*etx.Request, error)
	RequestHeadersByNumber(uint64, int, int, bool, chan *etx.Response) (*etx.Request, error)
}

// Peer encapsulates the metxods required to synchronise with a remote full peer.
type Peer interface {
	LightPeer
	RequestBodies([]common.Hash, chan *etx.Response) (*etx.Request, error)
	RequestReceipts([]common.Hash, chan *etx.Response) (*etx.Request, error)
}

// lightPeerWrapper wraps a LightPeer struct, stubbing out the Peer-only metxods.
type lightPeerWrapper struct {
	peer LightPeer
}

func (w *lightPeerWrapper) Head() (common.Hash, *big.Int) { return w.peer.Head() }
func (w *lightPeerWrapper) RequestHeadersByHash(h common.Hash, amount int, skip int, reverse bool, sink chan *etx.Response) (*etx.Request, error) {
	return w.peer.RequestHeadersByHash(h, amount, skip, reverse, sink)
}
func (w *lightPeerWrapper) RequestHeadersByNumber(i uint64, amount int, skip int, reverse bool, sink chan *etx.Response) (*etx.Request, error) {
	return w.peer.RequestHeadersByNumber(i, amount, skip, reverse, sink)
}
func (w *lightPeerWrapper) RequestBodies([]common.Hash, chan *etx.Response) (*etx.Request, error) {
	panic("RequestBodies not supported in light client mode sync")
}
func (w *lightPeerWrapper) RequestReceipts([]common.Hash, chan *etx.Response) (*etx.Request, error) {
...
func (p *peerConnection) UpdateHeaderRate(delivered int, elapsed time.Duration) {
	p.rates.Update(etx.BlockHeadersMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateBodyRate(delivered int, elapsed time.Duration) {
	p.rates.Update(etx.BlockBodiesMsg, elapsed, delivered)
}
...
func (p *peerConnection) UpdateReceiptRate(delivered int, elapsed time.Duration) {
	p.rates.Update(etx.ReceiptsMsg, elapsed, delivered)
}

// HeaderCapacity retrieves the peer's header download allowance based on its
// previously discovered throughput.
func (p *peerConnection) HeaderCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(etx.BlockHeadersMsg, targetRTT)
...
func (p *peerConnection) BodyCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(etx.BlockBodiesMsg, targetRTT)
...
func (p *peerConnection) ReceiptCapacity(targetRTT time.Duration) int {
	cap := p.rates.Capacity(etx.ReceiptsMsg, targetRTT)
...
// Lacks retrieves whetxer the hash of a blockchain item is on the peers lacking
// list (i.e. whetxer we know that the peer does not have it).
...
func newPeerSet() *peerSet {
	return &peerSet{
		...
		rates: msgrate.NewTrackers(log.New("proto", "etx")),
	}
}
...
//
// The metxod also sets the starting throughput values of the new peer to the
