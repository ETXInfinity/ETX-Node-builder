...
// the process. This is usually the case when Getx is shutting down and some events
...
// data on disk when two subchains are joined togetxer.
...
// The update might request whetxer to forcefully change the target, or only try to
// extend it and fail if it's not possible.
type headUpdate struct {
	header *types.Header // Header to update the sync target to
	force  bool          // Whetxer to force the update or only extend if possible
...
type backfiller interface {
	...
	// on initial startup.
	//
	// The metxod should return the last block header that has been successfully
...
//
// Opposed to the original ETX block synchronization which is trustless (and
...
// not from within (vs. legacy ETX sync).
//
...
type skeleton struct {
	db     etxdb.Database // Database backing the skeleton
...
func newSkeleton(db etxdb.Database, peers *peerSet, drop peerDropFn, filler backfiller) *skeleton {
	sk := &skeleton{
		db:         db,
		filler:     filler,
		peers:      peers,
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Getx is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Getx requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
...
// ancestor.
//
// This metxod does not block, rather it just waits until the syncer receives the
...
func (s *skeleton) sync(head *types.Header) (*types.Header, error) {
	...
	defer func() {
		if filled := s.filler.suspend(); filled != nil {
			// If sometxing was filled, try to delete stale sync helpers. If
	...
	for {
		// Sometxing happened, try to assign new tasks to any idle peers
		if !linked {
...
// is nil, the metxod will return and continue from the previous head.
	...
	if status := rawdb.ReadSkeletonSyncStatus(s.db); len(status) > 0 {
		...
		if err := json.Unmarshal(status, s.progress); err != nil {
			...
			for len(s.progress.Subchains) > 0 {
				// If the last chain is above the new head, delete altogetxer
...
func (s *skeleton) saveSyncStatus(db etxdb.KeyValueWriter) {
...
func (s *skeleton) assignTasks(success chan *headerResponse, fail chan *headerRequest, cancel chan struct{}) {
	...
	for _, peer := range s.idles {
		...
		idlers.caps = append(idlers.caps, s.peers.rates.Capacity(peer.id, etx.BlockHeadersMsg, targetTTL))
	}
...
// arrives or a timeouts / cancellation is triggered. The metxod should be run
...
func (s *skeleton) executeTask(peer *peerConnection, req *headerRequest) {
	...
	case <-timeoutTimer.C:
		...
		s.peers.rates.Update(peer.id, etx.BlockHeadersMsg, 0, 0)
		s.scheduleRevertRequest(req)
	...
	case res := <-resCh:
		...
		headers := *res.Res.(*etx.BlockHeadersPacket)

		headerReqTimer.Update(time.Since(start))
		s.peers.rates.Update(peer.id, etx.BlockHeadersMsg, res.Time, len(headers))
...
func (s *skeleton) processResponse(res *headerResponse) (linked bool, merged bool) {
	...
	// Whetxer the response is valid, we can mark the peer as idle and notify
...
func (s *skeleton) cleanStales(filled *types.Header) error {
	...
	// If the filled header is below the linked subchain, sometxing's
...
// This metxod is used by the backfiller, whose life cycle is controlled by the
// skeleton syncer.
//
// Note, the metxod will not use the internal state of the skeleton, but will
...
// of this metxod. Any head reorg will first tear down the backfiller and only
...
// Header retrieves a specific header tracked by the skeleton syncer. This metxod
// is meant to be used by the backfiller, whose life cycle is controlled by the
// skeleton syncer.
//
// Note, outside the permitted runtimes, this metxod might return nil results and
