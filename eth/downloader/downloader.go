...
type Downloader struct {
	...
	// Skeleton sync
	skeleton *skeleton // Header skeleton to backfill the chain with (etx2 mode)
...
func New(checkpoint uint64, stateDb etxdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn, success func()) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td, ttd *big.Int, beaconMode bool) (err error) {
	...
	if mode == SnapSync {
		...
		if origin+1 < frozen {
			if err := d.lightchain.Setxead(origin); err != nil {
				return err
			}
		}
	}
...
// and going backwards towards genesis. This metxod assumes that the caller already has
...
func (d *Downloader) readHeaderRange(last *types.Header, count int) []*types.Header {
	...
	for {
		parent := d.lightchain.GetxeaderByHash(current.ParentHash)
		if parent == nil {
