...
func (etx *ETX) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServicetxreads; i++ {
		go func() {
			for {
				select {
				case <-etx.closeBloomHandler:
					return
				...
				case request := <-etx.bloomRequests:
					...
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(etx.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(etx.chainDb, task.Bit, section, head); err == nil {
