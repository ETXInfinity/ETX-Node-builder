...
import (
	...
	"github.com/ETX/go-ETX/etx/fetcher"
...
// txPool defines the metxods needed from a transaction pool implementation to
// support all the operations needed by the ETX chain protocols.
...
type handlerConfig struct {
	Database       etxdb.Database            // Database for direct sync insertions
	Chain          *core.BlockChain          // Blockchain to serve data from
	TxPool         txPool                    // Transaction pool to propagate from
	Merger         *consensus.Merger         // The manager for etx1/2 transition
	Network        uint64                    // Network identifier to adfvertise
	Sync           downloader.SyncMode       // Whetxer to snap or full sync
...
type handler struct {
	...
	snapSync  uint32 // Flag whetxer snap sync is enabled (gets disabled if we already have blocks)
	acceptTxs uint32 // Flag whetxer we're considered synchronised (enables transaction processing)

	checkpointNumber uint64      // Block number for the sync progress validator to cross reference
	checkpointHash   common.Hash // Block hash for the sync progress validator to cross reference

	database etxdb.Database
	txpool   txPool
...
// newHandler returns a handler for all ETX chain management protocol.
func newHandler(config *handlerConfig) (*handler, error) {
	...
	inserter := func(blocks types.Blocks) (int, error) {
		...
		// out a way yet where nodes can decide unilaterally whetxer the network is new
		...
		if h.merger.TDDReached() {
			...
			// pow blocks. Spec: https://github.com/ETX/EIPs/blob/master/EIPS/eip-3675.md#halt-the-importing-of-pow-blocks
			for i, block := range blocks {
				...
				if err := h.chain.InsertBlockWithoutSetxead(block); err != nil {
					return i, err
				}
			}
			return 0, nil
...
// runetxPeer registers an etx peer into the joint etx/snap peerset, adds it to
// various subsystems and starts handling messages.
func (h *handler) runetxPeer(peer *etx.Peer, handler etx.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("ETX handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("ETX peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("ETX peer registration failed", "err", err)
		return err
	}
	...
	if err := h.downloader.RegisterPeer(peer.ID(), peer.Version(), peer); err != nil {
		peer.Log().Error("Failed to register peer in etx syncer", "err", err)
		return err
	}
	if snap != nil {
	...
	if h.checkpointHash != (common.Hash{}) {
		...
		go func() {
			...
			select {
			case res := <-resCh:
				headers := ([]*types.Header)(*res.Res.(*etx.BlockHeadersPacket))
				if len(headers) == 0 {
	...
	for number, hash := range h.requiredBlocks {
		...
		go func(number uint64, hash common.Hash, req *etx.Request) {
			// Ensure the request gets cancelled in case of error/drop
			defer req.Close()
...
// runSnapExtension registers a `snap` peer into the joint etx/snap peerset and
// starts handling inbound messages. As `snap` is only a satellite protocol to
// `etx`, all subsystem registrations and lifecycle management will be done by
// the main `etx` handler to prevent strange races.
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("ETX peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `etx` peer if it exists
	logger.Debug("Removing ETX peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("ETX peer removal failed", "err", err)
	}
}
...
func (h *handler) BroadcastTransactions(txs types.Transactions) {
	var (
		...
		txset = make(map[*etxPeer][]common.Hash) // Set peer->hash to transfer directly
		annos = make(map[*etxPeer][]common.Hash) // Set peer->hash to announce
