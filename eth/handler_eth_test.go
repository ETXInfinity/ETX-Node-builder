...
// testetxHandler is a mock event handler to listen for inbound network requests
// on the `etx` protocol and convert them into a more easily testable form.
type testetxHandler struct {
	blockBroadcasts event.Feed
	txAnnounces     event.Feed
	txBroadcasts    event.Feed
}

func (h *testetxHandler) Chain() *core.BlockChain              { panic("no backing chain") }
func (h *testetxHandler) TxPool() etx.TxPool                   { panic("no backing tx pool") }
func (h *testetxHandler) AcceptTxs() bool                      { return true }
func (h *testetxHandler) RunPeer(*etx.Peer, etx.Handler) error { panic("not used in tests") }
func (h *testetxHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testetxHandler) Handle(peer *etx.Peer, packet etx.Packet) error {
	switch packet := packet.(type) {
	case *etx.NewBlockPacket:
		h.blockBroadcasts.Send(packet.Block)
		return nil

	case *etx.NewPooledTransactionHashesPacket66:
		h.txAnnounces.Send(([]common.Hash)(*packet))
		return nil

	case *etx.NewPooledTransactionHashesPacket68:
		h.txAnnounces.Send(packet.Hashes)
		return nil

	case *etx.TransactionsPacket:
		h.txBroadcasts.Send(([]*types.Transaction)(*packet))
		return nil

	case *etx.PooledTransactionsPacket:
		h.txBroadcasts.Send(([]*types.Transaction)(*packet))
		return nil

	default:
		panic(fmt.Sprintf("unexpected etx packet type in tests: %T", packet))
	}
}

// Tests that peers are correctly accepted (or rejected) based on the advertised
// fork IDs in the protocol handshake.
func TestForkIDSplit66(t *testing.T) { testForkIDSplit(t, etx.etx66) }
func TestForkIDSplit67(t *testing.T) { testForkIDSplit(t, etx.etx67) }
func TestForkIDSplit68(t *testing.T) { testForkIDSplit(t, etx.etx68) }
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	var (
		...
		etxNoFork, _ = newHandler(&handlerConfig{
			Database:   dbNoFork,
			Chain:      chainNoFork,
		...
		etxProFork, _ = newHandler(&handlerConfig{
			Database:   dbProFork,
			Chain:      chainProFork,
			TxPool:     newTestTxPool(),
			Merger:     consensus.NewMerger(rawdb.NewMemoryDatabase()),
	...
	etxNoFork.Start(1000)
	etxProFork.Start(1000)

	// Clean up everything after ourselves
	defer chainNoFork.Stop()
	defer chainProFork.Stop()

	defer etxNoFork.Stop()
	defer etxProFork.Stop()
	...
	peerNoFork := etx.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pNoFork), p2pNoFork, nil)
	peerProFork := etx.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pProFork), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- etxNoFork.runetxPeer(peerProFork, func(peer *etx.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- etxProFork.runetxPeer(peerNoFork, func(peer *etx.Peer) error { return nil })
	}(errc)
	...
	peerNoFork = etx.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork = etx.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()
...
func TestRecvTransactions66(t *testing.T) { testRecvTransactions(t, etx.etx66) }
func TestRecvTransactions67(t *testing.T) { testRecvTransactions(t, etx.etx67) }
func TestRecvTransactions68(t *testing.T) { testRecvTransactions(t, etx.etx68) }
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := etx.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, handler.txpool)
	sink := etx.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runetxPeer(sink, func(peer *etx.Peer) error {
		return etx.Handle((*etxHandler)(handler.handler), peer)
	})
...
func TestSendTransactions66(t *testing.T) { testSendTransactions(t, etx.etx66) }
func TestSendTransactions67(t *testing.T) { testSendTransactions(t, etx.etx67) }
func TestSendTransactions68(t *testing.T) { testSendTransactions(t, etx.etx68) }
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runetxPeer(src, func(peer *etx.Peer) error {
		return etx.Handle((*etxHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
	...
	for len(seen) < len(insert) {
		...
		case 66, 67, 68:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post etx/66")
			}
...
func TestTransactionPropagation66(t *testing.T) { testTransactionPropagation(t, etx.etx66) }
func TestTransactionPropagation67(t *testing.T) { testTransactionPropagation(t, etx.etx67) }
func TestTransactionPropagation68(t *testing.T) { testTransactionPropagation(t, etx.etx68) }
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := etx.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{byte(i + 1)}, "", nil, sourcePipe), sourcePipe, source.txpool)
		sinkPeer := etx.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runetxPeer(sourcePeer, func(peer *etx.Peer) error {
			return etx.Handle((*etxHandler)(source.handler), peer)
		})
		go sink.handler.runetxPeer(sinkPeer, func(peer *etx.Peer) error {
			return etx.Handle((*etxHandler)(sink.handler), peer)
		})
	}
...
// Tests that post etx protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := etx.NewPeer(etx.etx66, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pLocal), p2pLocal, handler.txpool)
	remote := etx.NewPeer(etx.etx66, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pRemote), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()
	...
	go func() {
		...
		handler.handler.runetxPeer(local, func(peer *etx.Peer) error {
			return etx.Handle((*etxHandler)(handler.handler), peer)
		})
	}()
	...
	if checkpoint {
		...
		request := new(etx.GetBlockHeadersPacket66)
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	sinks := make([]*testetxHandler, peers)
	for i := 0; i < len(sinks); i++ {
		sinks[i] = new(testetxHandler)
	}
	// Interconnect all the sink handlers with the source handler
	var (
	...
	for i, sink := range sinks {
		...
		sourcePeer := etx.NewPeer(etx.etx66, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, nil)
		sinkPeer := etx.NewPeer(etx.etx66, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func TestBroadcastMalformedBlock66(t *testing.T) { testBroadcastMalformedBlock(t, etx.etx66) }
func TestBroadcastMalformedBlock67(t *testing.T) { testBroadcastMalformedBlock(t, etx.etx67) }
func TestBroadcastMalformedBlock68(t *testing.T) { testBroadcastMalformedBlock(t, etx.etx68) }
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := etx.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, source.txpool)
	sink := etx.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runetxPeer(src, func(peer *etx.Peer) error {
		return etx.Handle((*etxHandler)(source.handler), peer)
	})
