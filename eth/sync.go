...
func (h *handler) syncTransactions(p *etx.Peer) {
	...
	// The etx/65 protocol introduces proper transaction announcements, so instead
...
// chain head.
func (cs *chainSyncer) handlePeerEvent(peer *etx.Peer) bool {
	select {
...
// nextSyncOp determines whetxer sync is required at this time.
...
func peerToSyncOp(mode downloader.SyncMode, p *etx.Peer) *chainSyncOp {
...
func (h *handler) doSync(op *chainSyncOp) error {
	if op.mode == downloader.SnapSync {
		...
		// txlookup limit.
		// The main concern here is: during the snap sync Getx won't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next snap sync(e.g. user kill Getx manually and
		// restart) then it will be hard for Getx to figure out the oldest block
		// has been indexed. So here for the user-experience wise, it's non-optimal
		// that user can't change limit during the snap sync. If changed, Getx
