...
// information related to the ETX protocol such als blocks, transactions and logs.
...
// `etx_getFilterChanges` polling metxod that is also used for log filters.
			...
			case txs := <-txs:
				...
				for _, tx := range txs {
					if fullTx != nil && *fullTx {
						rpcTx := etxapi.NewRPCPendingTransaction(tx, latest, chainConfig)
...
// It is part of the filter package since polling goes with etx_getFilterChanges.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(ETX.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as ETX.FilterQuery but with UnmarshalJSON() metxod.
type FilterCriteria ETX.FilterQuery

// NewFilter creates a new filter and returns the filter id. It can be
// used to retrieve logs when the state changes. This metxod cannot be
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(ETX.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
