...
// Package filters implements an ETX filtering system for block,
...
func (sub *Subscription) Unsubscribe() {
	...
	uninstallLoop:
		for {
			...
			// the eventLoop broadcast metxod to deadlock when writing to the
			// filter event channel while the subscription loop is waiting for
			// this metxod to return (and thus not reading these events).
			select {
		...
		// will probably be closed by the client asap after this metxod returns.
		<-sub.Err()
	})
}
...
func (es *EventSystem) SubscribeLogs(crit ETX.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit ETX.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit ETX.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit ETX.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
