...
func BenchmarkFilters(b *testing.B) {
	var (
		...
		addr3   = common.BytesToAddress([]byte("ETX"))
	...
	defer db.Close()
	_, chain, receipts := core.GenerateChainWithGenesis(gspec, etxash.NewFaker(), 100010, func(i int, gen *core.BlockGen) {
		switch i {
		case 2403:
...
func TestFilters(t *testing.T) {
	...
	_, chain, receipts := core.GenerateChainWithGenesis(gspec, etxash.NewFaker(), 1000, func(i int, gen *core.BlockGen) {
		switch i {
		case 1:
			receipt := types.NewReceipt(nil, false, 0)
			receipt.Logs = []*types.Log{
