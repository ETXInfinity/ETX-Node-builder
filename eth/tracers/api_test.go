...
func (b *testBackend) HeaderByHash(ctx context.Context, hash common.Hash) (*types.Header, error) {
	return b.chain.GetxeaderByHash(hash), nil
}
...
func (b *testBackend) HeaderByNumber(ctx context.Context, number rpc.BlockNumber) (*types.Header, error) {
	...
	return b.chain.GetxeaderByNumber(uint64(number)), nil
}
...
func TestTraceCall(t *testing.T) {
	...
	genesis := &core.Genesis{
		...
		Alloc: core.GenesisAlloc{
			accounts[0].addr: {Balance: big.NewInt(params.etxer)},
			accounts[1].addr: {Balance: big.NewInt(params.etxer)},
			accounts[2].addr: {Balance: big.NewInt(params.etxer)},
		},
	}
	genBlocks := 10
	...
	var testSuite = []struct {
		...
		},
		{
			...
			config: &TraceCallConfig{
				BlockOverrides: &etxapi.BlockOverrides{Number: (*hexutil.Big)(big.NewInt(0x1337))},
			},
			expectErr: nil,
...
func TestTracingWithOverrides(t *testing.T) {
	...
	var testSuite = []struct {
		...
		// Call which can only succeed if state is state overridden
		{
			...
			config: &TraceCallConfig{
				StateOverrides: &etxapi.StateOverride{
					randomAccounts[0].addr: etxapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.etxer)))},
				},
			},
		...
		//  }
		{
			...
			config: &TraceCallConfig{
				//Tracer: &tracer,
				StateOverrides: &etxapi.StateOverride{
					randomAccounts[2].addr: etxapi.OverrideAccount{
