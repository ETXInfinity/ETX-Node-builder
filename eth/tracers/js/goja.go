...
import (
	...
	jsassets "github.com/ETX/go-ETX/etx/tracers/js/internal/tracers"
)
...
// init retrieves the JavaScript transaction tracers included in go-ETX.
func init() {
	var err error
...
type jsTracer struct {
	...
	traceStep         bool                  // True if tracer object exposes a `step()` metxod
	traceFrame        bool                  // True if tracer object exposes the `enter()` and `exit()` metxods
	gasLimit          uint64                // Amount of gas bought for the whole tx
	err               error                 // Any error that should stop tracing
	obj               *goja.Object          // Trace object

	// Metxods exposed by tracer
...
// evaluates to an expression returning an object with certain metxods.
// The metxods `result` and `fault` are required to be present.
// The metxods `step`, `enter`, and `exit` are optional, but note that
// `enter` and `exit` always go togetxer.
func newJsTracer(code string, ctx *tracers.Context, cfg json.RawMessage) (tracers.Tracer, error) {
	...
	// Check tracer's interface for required and optional metxods.
	obj := ret.ToObject(vm)
