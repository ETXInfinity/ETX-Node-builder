...
// etxPeerInfo represents a short summary of the `etx` sub-protocol metadata known
// about a connected peer.
type etxPeerInfo struct {
	Version    uint     `json:"version"`    // ETX protocol version negotiated
	Difficulty *big.Int `json:"difficulty"` // Total difficulty of the peer's blockchain
	Head       string   `json:"head"`       // Hex hash of the peer's best owned block
}

// etxPeer is a wrapper around etx.Peer to maintain a few extra metadata.
type etxPeer struct {
	*etx.Peer
	snapExt *snapPeer // Satellite `snap` connection
}

// info gathers and returns some `etx` protocol metadata known about a peer.
func (p *etxPeer) info() *etxPeerInfo {
	hash, td := p.Head()
