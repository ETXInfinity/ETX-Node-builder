...
func (p *hookedPrompter) Setxistory(history []string)                     {}
...
func newTester(t *testing.T, confOverride func(*etxconfig.Config)) *tester {
	// Create a temporary storage for the node keys and initialize it
	workspace := t.TempDir()

	// Create a networkless protocol stack and start an ETX service within
	...
	etxConf := &etxconfig.Config{
		...
		Miner: miner.Config{
			etxerbase: common.HexToAddress(testAddress),
		},
		etxash: etxash.Config{
			PowMode: etxash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(etxConf)
	}
	etxBackend, err := etx.New(stack, etxConf)
	if err != nil {
		t.Fatalf("failed to register ETX protocol: %v", err)
	}
