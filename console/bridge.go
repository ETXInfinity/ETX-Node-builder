...
import (
	...
	"github.com/ETX/go-ETX/accounts/usbwallet"
	"github.com/ETX/go-ETX/common/hexutil"
	"github.com/ETX/go-ETX/console/prompt"
	"github.com/ETX/go-ETX/internal/jsre"
	"github.com/ETX/go-ETX/rpc"
)

// bridge is a collection of JavaScript utility metxods to bride the .js runtime
// environment and the Go RPC connection backing the remote metxod calls.
type bridge struct {
	client   *rpc.Client         // RPC client to execute ETX requests through
...
func getJetx(vm *goja.Runtime) *goja.Object {
	jetx := vm.Get("jetx")
	if jetx == nil {
		panic(vm.ToValue("jetx object does not exist"))
	}
	return jetx.ToObject(vm)
}

// NewAccount is a wrapper around the personal.newAccount RPC metxod that uses a
// non-echoing password prompt to acquire the passphrase and executes the original
// RPC metxod (saved in jetx.newAccount) with it to actually execute the RPC call.
func (b *bridge) NewAccount(call jsre.Call) (goja.Value, error) {
	...
	newAccount, callable := goja.AssertFunction(getJetx(call.VM).Get("newAccount"))
	if !callable {
		return nil, fmt.Errorf("jetx.newAccount is not callable")
	}
...
func (b *bridge) OpenWallet(call jsre.Call) (goja.Value, error) {
	...
	openWallet, callable := goja.AssertFunction(getJetx(call.VM).Get("openWallet"))
	if !callable {
		return nil, fmt.Errorf("jetx.openWallet is not callable")
	}
...
// UnlockAccount is a wrapper around the personal.unlockAccount RPC metxod that
// uses a non-echoing password prompt to acquire the passphrase and executes the
// original RPC metxod (saved in jetx.unlockAccount) with it to actually execute
// the RPC call.
func (b *bridge) UnlockAccount(call jsre.Call) (goja.Value, error) {
	...
	unlockAccount, callable := goja.AssertFunction(getJetx(call.VM).Get("unlockAccount"))
	if !callable {
		return nil, fmt.Errorf("jetx.unlockAccount is not callable")
	}
	return unlockAccount(goja.Null(), account, passwd, duration)
}

// Sign is a wrapper around the personal.sign RPC metxod that uses a non-echoing password
// prompt to acquire the passphrase and executes the original RPC metxod (saved in
// jetx.sign) with it to actually execute the RPC call.
func (b *bridge) Sign(call jsre.Call) (goja.Value, error) {
	...
	// Send the request to the backend and return
	sign, callable := goja.AssertFunction(getJetx(call.VM).Get("sign"))
	if !callable {
		return nil, fmt.Errorf("jetx.sign is not callable")
	}
...
func (b *bridge) SleepBlocks(call jsre.Call) (goja.Value, error) {
	...
	if err := b.client.Call(&lastNumber, "etx_blockNumber"); err != nil {
		return nil, err
	}
	for time.Now().Before(deadline) {
		var number hexutil.Uint64
		if err := b.client.Call(&number, "etx_blockNumber"); err != nil {
			return nil, err
		}
...
// Send implements the web3 provider "send" metxod.
func (b *bridge) Send(call jsre.Call) (goja.Value, error) {
	...
	for _, req := range reqs {
		...
		if err = b.client.Call(&result, req.Metxod, req.Params...); err == nil {
			if result == nil {
