...
import (
	...
	"github.com/ETX/go-ETX/internal/jsre/deps"
	"github.com/ETX/go-ETX/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	if c.prompter != nil {
		if content, err := os.ReadFile(c.histPath); err != nil {
			c.prompter.Setxistory(nil)
		} else {
			c.history = strings.Split(string(content), "\n")
			c.prompter.Setxistory(c.history)
		}
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"etx": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
// initPersonal redirects account-related API metxods through the bridge.
//
// If the console is in interactive mode and the 'personal' API is available, override
// the openWallet, unlockAccount, newAccount and sign metxods since these require user
// interaction. The original web3 callbacks are stored in 'jetx'. These will be called
...
func (c *Console) initPersonal(vm *goja.Runtime, bridge *bridge) {
	...
	jetx := vm.NewObject()
	vm.Set("jetx", jetx)
	jetx.Set("openWallet", personal.Get("openWallet"))
	jetx.Set("unlockAccount", personal.Get("unlockAccount"))
	jetx.Set("newAccount", personal.Get("newAccount"))
	jetx.Set("sign", personal.Get("sign"))
...
// consoleOutput is an override for the console.log and console.error metxods to
...
// input prompter to provide hints to the user about the metxods available.
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines etx.getBalance(etx.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
		// Skip all metxods and namespaces (i.e. including the dot)
...
// Welcome show summary of current Getx instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Getx JavaScript console!\n\n"

	// Print some generic Getx metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + etx.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + etx.blockNumber + " (" + new Date(1000 * etx.getBlock(etx.blockNumber).timestamp) + ")\n";
		try {
