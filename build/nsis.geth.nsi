...
# 1. install getx for all users
# 2. install optional development tools such as abigen
# 3. create an uninstaller
# 4. configures the Windows firewall for getx
# 5. create getx, attach and uninstall start menu entries
# 6. configures the registry that allows Windows to manage the package through its platform tools
# 7. adds the environment system wide variable ETX_SOCKET
...
!define GROUPNAME "ETX"
!define APPNAME "Getx"
!define DESCRIPTION "Official Go implementation of the ETX protocol"
!addplugindir .\
...
function .onInit
  # make vars are global for all users since getx is installed global
