...
const ourPath = "github.com/ETX/go-ETX" // Path to our module
...
// conventions in the ETX p2p network.
...
//
// If the package that is currently executing is a prefixed by our go-ETX
...
// version and whetxer it was replaced by another module.
...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "getx".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("getx %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("getx %s", params.VersionWithMeta)
	}
	...
	if mod.Replace != nil {
		// If our package was replaced by sometxing else, also note that.
