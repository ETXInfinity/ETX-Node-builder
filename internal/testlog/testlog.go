...
// t.Logf(). All metxods in between logger.Trace, logger.Debug, etc. are marked as test
...
func Logger(t *testing.T, level log.Lvl) log.Logger {
	...
	l.l.Setxandler(log.LvlFilterHandler(level, l.h))
	return l
}
...
func (l *logger) Getxandler() log.Handler {
	return l.l.Getxandler()
}

func (l *logger) Setxandler(h log.Handler) {
	l.l.Setxandler(h)
}
