...
type TestCmd struct {
	// For total convenience, all testing metxods are available.
	*testing.T
...
// reexec init function for that name (e.g. "getx-test" in cmd/getx/run_test.go)
...
// This metxod can also be called from an expect template, e.g.:
//
//	getx.expect(`Passphrase: {{.InputLine "password"}}`)
