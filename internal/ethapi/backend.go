...
// Package etxapi implements the general ETX API functions.
package etxapi
...
type Backend interface {
	...
	ExtRPCEnabled() bool
	RPCGasCap() uint64            // global gas cap for etx_call over rpc: DoS protection
	RPCEVMTimeout() time.Duration // global timeout for etx_call over rpc: DoS protection
	RPCTxFeeCap() float64         // global tx fee cap for all transaction related APIs
	UnprotectedAllowed() bool     // allows only for EIP155 transactions.

	// Blockchain API
	Setxead(number uint64)
	...
	// etx/filters needs to be initialized from this backend type, so metxods needed by
	...
	return []rpc.API{
		{
			...
			Namespace: "etx",
			Service:   NewETXAccountAPI(apiBackend.AccountManager()),
		}, {
			Namespace: "personal",
