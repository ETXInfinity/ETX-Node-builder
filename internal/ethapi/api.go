...
import (
	...
	"github.com/ETX/go-ETX/accounts/scwallet"
...
// ETXAPI provides an API to access ETX related information.
type ETXAPI struct {
	b Backend
}

// NewETXAPI creates a new ETX protocol API.
func NewETXAPI(b Backend) *ETXAPI {
	return &ETXAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *ETXAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *ETXAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *ETXAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *ETXAPI) Syncing() (interface{}, error) {
...
// ETXAccountAPI provides an API to access accounts managed by this node.
// It offers only metxods that can retrieve accounts.
type ETXAccountAPI struct {
	am *accounts.Manager
}

// NewETXAccountAPI creates a new ETXAccountAPI.
func NewETXAccountAPI(am *accounts.Manager) *ETXAccountAPI {
	return &ETXAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *ETXAccountAPI) Accounts() []common.Address {
	return s.am.Accounts()
}

// PersonalAccountAPI provides an API to access accounts managed by this node.
// It offers metxods to create, (un)lock en list accounts. Some metxods accept
...
// the metxod may return an extra challenge requiring a second open (e.g. the
...
// Sign calculates an ETX ECDSA signature for:
// keccak256("\x19ETX Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/ETX/go-ETX/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with etx_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19ETX Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/ETX/go-ETX/wiki/Management-APIs#personal_ecRecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid ETX signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and getx.
...
// BlockChainAPI provides an API to access ETX blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new ETX blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current ETX chain config.
//
// Note, this metxod does not conform to EIP-695 because the configured chain ID is always
...
// GetxeaderByNumber returns the requested canonical block header.
// * When blockNr is -1 the chain head is returned.
// * When blockNr is -2 the pending chain head is returned.
func (s *BlockChainAPI) GetxeaderByNumber(ctx context.Context, number rpc.BlockNumber) (map[string]interface{}, error) {
...
// GetxeaderByHash returns the requested header by hash.
func (s *BlockChainAPI) GetxeaderByHash(ctx context.Context, hash common.Hash) map[string]interface{} {
...
// See: https://github.com/ETX/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
// TransactionAPI exposes metxods for reading and creating transaction data.
...
// NewTransactionAPI creates a new RPC service with metxods for interacting with transactions.
...
func (s *TransactionAPI) GetTransactionReceipt(ctx context.Context, hash common.Hash) (map[string]interface{}, error) {
	tx, blockHash, blockNumber, index, err := s.b.GetTransaction(ctx, hash)
	if err != nil {
		// When the transaction doesn't exist, the RPC metxod should return JSON null
...
//
// https://github.com/ETX/wiki/wiki/JSON-RPC#etx_sign
...
// DebugAPI is the collection of ETX APIs exposed over the debugging
// namespace.
...
func (api *DebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("%#x", etxash.SeedHash(number)), nil
}
...
// Setxead rewinds the head of the blockchain to a previous block.
func (api *DebugAPI) Setxead(number hexutil.Uint64) {
	api.b.Setxead(uint64(number))
}

// NetAPI offers network related RPC metxods
type NetAPI struct {
	net            *p2p.Server
...
// Version returns the current ETX protocol version.
func (s *NetAPI) Version() string {
	return fmt.Sprintf("%d", s.networkVersion)
}

// checkTxFee is an internal function used to check whetxer the fee of
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeetx := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.etxer)))
	feeFloat, _ := feeetx.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f etxer) exceeds the configured cap (%.2f etxer)", feeFloat, cap)
	}
	return nil
}
